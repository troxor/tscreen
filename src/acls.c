...
static struct aclusergroup **FindGroupPtr
__P((struct aclusergroup **, struct acluser *, int));
static int AclSetPermCmd __P((struct acluser *, char *, struct comm *));
static int AclSetPermWin
__P((struct acluser *, struct acluser *, char *, struct win *));
static int UserAcl
__P((struct acluser *, struct acluser **, int, char **));
static int UserAclCopy __P((struct acluser **, struct acluser **));


static int GrowBitfield(bfp, len, delta, defaultbit)
AclBits *bfp;
 ...
 */
struct acluser **FindUserPtr(name)
char *name;
{
	struct acluser **u;
...
struct acluser **up;
{
	...
	for ((*up)->u_id = 0; (*up)->u_id < maxusercount; (*up)->u_id++)
		if (!
		    (ACLBIT((*up)->u_id) & ACLBYTE(userbits, (*up)->u_id)))
			break;
...
*/
		...
		for (j = 0; j <= RC_LAST; j++) {
			...
			for (i = 0; i < ACL_BITS_PER_CMD; i++)
				if (GrowBitfield
				    (&comms[j].userbits[i], maxusercount,
				     USER_CHUNK, default_c_bit[i])) {
					free((char *) *up);
					*up = NULL;
					return -1;
				}
		...
		for (u = users; u != *up; u = u->u_next) {
			for (j = 0; j < ACL_BITS_PER_WIN; j++) {
				if (GrowBitfield
				    (&u->u_umask_w_bits[j], maxusercount,
				     USER_CHUNK, default_w_bit[j])) {
					free((char *) *up);
					*up = NULL;
					return -1;
				}
		...
		for (w = windows; w; w = w->w_next) {
			...
			if (GrowBitfield
			    (&w->w_mon_notify, maxusercount, USER_CHUNK, 0)
	...
	for (j = 0; j < ACL_BITS_PER_WIN; j++) {
		if (GrowBitfield
		    (&(*up)->u_umask_w_bits[j], 0, maxusercount,
		     default_w_bit[j])) {
			free((char *) *up);
			*up = NULL;
			return -1;
		}
		ACLBYTE((*up)->u_umask_w_bits[j], (*up)->u_id) |=
		    ACLBIT((*up)->u_id);
	}
#else				/* MULTIUSER */
	debug1("UserAdd %s\n", name);
 ...
 */
static struct aclusergroup **FindGroupPtr(gp, u, recursive)
...
int recursive;
{
	...
	while (*gp) {
		...
		if (recursive &&
		    *(g =
		      FindGroupPtr(&(*gp)->u->u_group, u, recursive + 1)))
...
char *from, *to;
{
	...
	if (!
	    (*g =
	     (struct aclusergroup *) malloc(sizeof(struct aclusergroup))))
...
char *name, *pw1, *pw2;
{
	...
	debug2("syslog(LOG_NOTICE, \"screen %s: \"su %s\" ", SockPath,
	       name);
	debug2("%s for \"%s\"\n", sorry ? "failed" : "succeded",
	       (*up)->u_name);
#ifdef SYSLOG
#ifdef BSD_42
 ...
 */
static int AclSetPermCmd(u, mode, cmd)
struct acluser *u;
char *mode;
struct comm *cmd;
{
...
/*	  debug3("AclSetPermCmd %s %s %s\n", u->u_name, mode, cmd->name); */
			if (neg)
				ACLBYTE(cmd->userbits[ACL_EXEC],
					u->u_id) &= ~ACLBIT(u->u_id);
			else
				ACLBYTE(cmd->userbits[ACL_EXEC],
					u->u_id) |= ACLBIT(u->u_id);
			break;
 ...
 */
static int AclSetPermWin(uu, u, mode, win)
struct acluser *u, *uu;
char *mode;
struct win *win;
{
	...
	while (*s) {
		...
		case '#':
			if (uu)	/* window umask or .. */
				AclSetPermWin(uu, u, mode,
					      (struct win *) 1);
			else	/* .. or all windows */
				for (w = windows; w; w = w->w_next)
					AclSetPermWin((struct acluser *) 0,
						      u, mode, w);
			s++;
			break;
		case '?':
 ...
 * uu should be NULL, except if you want to change his umask.
 */
static int UserAcl(uu, u, argc, argv)
struct acluser *uu, **u;
int argc;
char **argv;
{
	...
	switch (argc) {
	case 1 + 1 + 2:
		debug2("UserAcl: user '%s', password '%s':", argv[0],
		       argv[1]);
...
static int UserAclCopy(to_up, from_up)
struct acluser **to_up, **from_up;
{
	...
	for (w = windows; w; w = w->w_next) {
		for (i = 0; i < ACL_BITS_PER_WIN; i++) {
			if (ACLBYTE(w->w_userbits[i], from_id) &
			    ACLBIT(from_id))
				ACLBYTE(w->w_userbits[i], to_id) |=
				    ACLBIT(to_id);
			else {
				ACLBYTE(w->w_userbits[i], to_id) &=
				    ~ACLBIT(to_id);
	...
	for (j = 0; j <= RC_LAST; j++) {
		for (i = 0; i < ACL_BITS_PER_CMD; i++) {
			if (ACLBYTE(comms[j].userbits[i], from_id) &
			    ACLBIT(from_id))
				ACLBYTE(comms[j].userbits[i], to_id) |=
				    ACLBIT(to_id);
			else
				ACLBYTE(comms[j].userbits[i], to_id) &=
				    ~ACLBIT(to_id);
...
struct win *w;
{
	...
	debug3("AclCheckPermWin(%s, %d, %d) = ", u->u_name, mode,
	       w->w_number);
