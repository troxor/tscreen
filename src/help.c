...
void exit_with_usage(myname, message, arg)
char *myname, *message, *arg;
{
	...
	printf
	    ("-c file       Read configuration file instead of '.tscreenrc'.\n");
	printf
	    ("-d (-r)       Detach the elsewhere running screen (and reattach here).\n");
	printf
	    ("-dmS name     Start as daemon: tscreen session in detached mode.\n");
	printf
	    ("-D (-r)       Detach and logout remote (and reattach here).\n");
	printf
	    ("-D -RR        Do whatever is needed to get a tscreen session.\n");
...
#if defined(UTMP_LOGOUTOK) && defined(UTMPOK)
	...
	printf
	    ("-m            ignore $STY variable, do create a new tscreen session.\n");
	printf
	    ("-O            Choose optimal output rather than exact vt100 emulation.\n");
	printf("-p window     Preselect the named window if it exists.\n");
	printf
	    ("-q            Quiet startup. Exits with non-zero return code if unsuccessful.\n");
	printf("-r            Reattach to a detached tscreen process.\n");
	printf
	...
	printf
	    ("-T term       Use term as $TERM for windows, rather than \"tscreen\".\n");
#ifdef UTF8
	printf("-U            Tell tscreen to use UTF-8 encoding.\n");
#endif
	printf("-v            Print \"tscreen version %s\".\n", version);
	printf("-wipe         Do nothing, just clean up SockDir.\n");
#ifdef MULTI
	printf
	    ("-x            Attach to a not detached tscreen. (Multi display mode).\n");
#endif				/* MULTI */
	printf
	    ("-X            Execute <cmd> as a tscreen command in the specified session.\n");
	if (message && *message) {
		printf("\nError: ");
		printf(message, arg);
		printf("\n");
...
struct action *ktabp;
{
	...
	helpdata->inter =
	    (flayer->l_width -
	     (mcom + mkey) * helpdata->numcols) / (helpdata->numcols + 1);
	if (helpdata->inter <= 0)
		helpdata->inter = 1;
	debug1("inter: %d\n", helpdata->inter);
	helpdata->mcom = mcom;
	helpdata->mkey = mkey;
	helpdata->numrows =
	    (helpdata->num_names + helpdata->numcols -
	     1) / helpdata->numcols;
	debug1("Numrows: %d\n", helpdata->numrows);
	...
	if (helpdata->numskip > flayer->l_height / 3
		...
		helpdata->numskip = 1;
	helpdata->maxrow =
	    2 + helpdata->numrows + helpdata->numskip +
	    helpdata->command_bindings;
	helpdata->grow = 0;

	helpdata->numpages =
	    (helpdata->maxrow + flayer->l_height - 6) / (flayer->l_height -
							 5);
	flayer->l_x = 0;
...
static void HelpProcess(ppbuf, plen)
char **ppbuf;
int *plen;
{
	int done = 0;
...
static int helppage()
{
	...
	sprintf(cbuf, "tscreen key bindings, page %d of %d.",
		helpdata->grow / (flayer->l_height - 5) + 1,
		helpdata->numpages);
	centerline(cbuf, 0);
	crow = 2;
	...
	*buf = '\0';
	...
	if (flayer->l_cvlist && flayer->l_cvlist->c_display) {
		add_key_to_buf(buf,
			       flayer->l_cvlist->c_display->d_user->
			       u_MetaEsc);
		add_key_to_buf(Esc_buf,
	...
	for (; crow < flayer->l_height - 3; crow++) {
		if (helpdata->grow < 1) {
			if (ktabp == ktab)
				...
				sprintf(cbuf, "Command class: '%.80s'",
					helpdata->class);
			centerline(cbuf, crow);
			helpdata->grow++;
		} else if (helpdata->grow >= 2
			   && helpdata->grow - 2 < helpdata->numrows) {
			x = 0;
			for (col = 0;
			     col < helpdata->numcols
			     && (n =
				 helpdata->numrows * col +
				 (helpdata->grow - 2)) <
			     helpdata->num_names; col++) {
				x += helpdata->inter - !col;
				n = helpdata->nact[n];
				buf[0] = '\0';
				for (key = 0; key < 256 + KMAP_KEYS; key++)
					if (ktabp[key].nr == n
					    && ktabp[key].args == noargs
					    && strlen(buf) <
					    sizeof(buf) - 7) {
						strcat(buf, " ");
						add_key_to_buf(buf, key);
					}
				PadStr(comms[n].name, helpdata->mcom, x,
				       crow);
				x += helpdata->mcom;
			...
			helpdata->grow++;
		} else if (helpdata->grow - 2 - helpdata->numrows >=
			   helpdata->numskip
			   && helpdata->grow - 2 - helpdata->numrows -
			   helpdata->numskip <
			   helpdata->command_bindings) {
			while ((n =
				ktabp[helpdata->command_search].nr) ==
			       RC_ILLEGAL
			       || ktabp[helpdata->command_search].args ==
			       noargs) {
			...
			PadStr(buf, 5, 0, crow);
			AddAction(&ktabp[helpdata->command_search++], 5,
				  crow);
			helpdata->grow++;
		} else
			helpdata->grow++;
	...
	sprintf(cbuf, "[Press Space %s Return to end.]",
		helpdata->grow <
		helpdata->maxrow ? "for next page;" : "or");
	centerline(cbuf, flayer->l_height - 2);
	LaySetCursor();
	return 0;
}

static void AddAction(act, x, y)
struct action *act;
int x, y;
{
	char buf[256];
	...
	while (pp && (cp = *pp) != NULL) {
		...
		*bp = 0;
		...
		pp++;
		if (*pp)
			LPutChar(flayer, fr ? &mchar_blank : &mchar_dol,
				 x++, y);
	}
}

static void add_key_to_buf(buf, key)
char *buf;
int key;
{
	buf += strlen(buf);
...
static void HelpRedisplayLine(y, xs, xe, isblank)
int y, xs, xe, isblank;
{
	if (y < 0) {
		struct helpdata *helpdata;
...
static void CopyrightProcess(ppbuf, plen)
char **ppbuf;
int *plen;
{
	int done = 0;
...
static void CopyrightRedisplayLine(y, xs, xe, isblank)
int y, xs, xe, isblank;
{
	ASSERT(flayer);
	if (y < 0) {
...
static void DisplaysProcess(ppbuf, plen)
char **ppbuf;
int *plen;
{
	int done = 0;
...
static void displayspage()
{
	...
	for (d = displays; d; d = d->d_next) {
		...
		sprintf(tbuf, "%-10.10s%4dx%-4d%10.10s@%-16.16s%s",
			d->d_termname, d->d_width, d->d_height,
			d->d_user->u_name, d->d_usertty, (d->d_blocked
							  || d->
							  d_nonblock >= 0)
			&& d->d_blocked <=
			4 ? blockstates[d->d_blocked] : "  ");
...
static void DisplaysRedisplayLine(y, xs, xe, isblank)
int y, xs, xe, isblank;
{
	ASSERT(flayer);
	if (y < 0) {
...
static void WListProcess(ppbuf, plen)
char **ppbuf;
int *plen;
{
	int done = 0;
	...
	h = wlistdata->numwin;
	while (!done && *plen > 0) {
		if ((unsigned char) **ppbuf >= '0'
		    && (unsigned char) **ppbuf <= '9') {
			int n = (unsigned char) **ppbuf - '0';
			int d = 0;
			if (n < MAXWIN && wtab[n] && WTAB_GROUP_MATCHES(n)) {
				int i;
				for (d = -wlistdata->npos, i =
				     WListNext(wlistdata, -1, 0); i != n;
		...
		case ' ':
			h = wlistdata->pos;
			if (h == MAXWIN && Layer2Window(flayer)
			    && Layer2Window(flayer)->w_type ==
			    W_TYPE_GROUP)
				break;
			if (display && h != MAXWIN && wtab[h]
			    && (wtab[h]->w_type == W_TYPE_GROUP
				|| wtab[h] == D_fore)) {
			...
			if (!display || h == MAXWIN || !wtab[h]
			    || wtab[h] == D_fore || (flayer->l_cvlist
						     && flayer->l_cvlist->
						     c_lnext))
				HelpAbort();
#ifdef MULTIUSER
			else if (AclCheckPermWin
				 (D_user, ACL_READ, wtab[h]))
				HelpAbort();
#endif
			else
		...
		case 0007:
			...
			if (h >= 0 && wtab[h])
				...
				struct win *p =
				    FindNiceWindow(display ? D_other
						   : (struct win *) 0, 0);
				if (p)
...
static void WListLine(y, i, pos, isblank)
int y, i;
int pos;
int isblank;
{
	...
	display =
	    Layer2Window(flayer) ? 0 : flayer->l_cvlist ? flayer->
	    l_cvlist->c_display : 0;
	str =
	    MakeWinMsgEv(wliststr, wtab[i], '%', flayer->l_width - xoff,
			 (struct event *) 0, 0);
	n = strlen(str);
	if (i != pos && isblank)
		while (n && str[n - 1] == ' ')
			n--;
	LPutWinMsg(flayer, str,
		   (i == pos
		    || !isblank) ? flayer->l_width : n,
		   i == pos ? &mchar_so : &mchar_blank, xoff, y + yoff);
	if (xoff)
		LPutWinMsg(flayer, "", xoff,
			   i == pos ? &mchar_so : &mchar_blank, 0,
			   y + yoff);
#if 0
	LPutStr(flayer, str, n, i == pos ? &mchar_so : &mchar_blank, 0,
		y + yoff);
	if (i == pos || !isblank)
		while (n < flayer->l_width)
			LPutChar(flayer,
				 i == pos ? &mchar_so : &mchar_blank, n++,
				 y + yoff);
#endif
	return;
}
...
static int WListNext(wlistdata, old, delta)
struct wlistdata *wlistdata;
int old, delta;
{
	int i;
...
static void WListLines(up, oldpos)
int up, oldpos;
{
	struct wlistdata *wlistdata;
	int ypos, pos;
	...
	for (y = 0; y < wlistdata->numwin; y++) {
		...
		if (y == 0)
			wlistdata->first = i;
		wlistdata->last = i;
		if (((i == oldpos || i == pos) && pos != oldpos)
		    || (up > 0 && y >= wlistdata->numwin - up) || (up < 0
								   && y <
								   -up))
...
static int WListNormalize()
{
	...
	for (n = 0, oldi = MAXWIN, i = pos;
	     i != MAXWIN && i != oldi && n < wlistdata->numwin;
	     oldi = i, i = WListNext(wlistdata, i, 1))
		n++;
	if (ypos < wlistdata->numwin - n)
		ypos = wlistdata->numwin - n;
	for (n = 0, oldi = MAXWIN, i = WListNext(wlistdata, -1, 0);
	     i != MAXWIN && i != oldi && i != pos;
	     oldi = i, i = WListNext(wlistdata, i, 1))
		n++;
	if (ypos > n)
		ypos = n;
...
static void WListMove(num, ypos)
int num;
int ypos;
{
	struct wlistdata *wlistdata;
...
static void WListRedisplayLine(y, xs, xe, isblank)
int y, xs, xe, isblank;
{
	ASSERT(flayer);
	if (y < 0) {
...
static int WListOrder(wlistdata, ind, start, group)
struct wlistdata *wlistdata;
int ind, start;
struct win *group;
{
	...
	if (!WTAB_GROUP_MATCHES(start)) {
		while (start < MAXWIN
		       && (!wtab[start] || !WTAB_GROUP_MATCHES(start)))
			if (wlistdata->order != WLIST_MRU)
				start++;
...
void display_wlist(onblank, order, group)
int onblank;
int order;
struct win *group;
{
...
static void wlistpage()
{
	...
	display = 0;
	str =
	    MakeWinMsgEv(wlisttit, (struct win *) 0, '%', flayer->l_width,
			 (struct event *) 0, 0);
	if (wlistdata->group) {
		LPutWinMsg(flayer, "Group: ", 7, &mchar_blank, 0, 0);
		LPutWinMsg(flayer, wlistdata->group->w_title,
			   strlen(wlistdata->group->w_title), &mchar_blank,
			   7, 0);
...
struct win *p;
{
	...
	if (wlistdata->order == WLIST_NUM
	    && (n < wlistdata->first || n > wlistdata->last))
		return;
	i = wlistdata->first;
	for (y = 0; y < wlistdata->numwin; y++) {
		if (i == n)
...
void WListLinkChanged()
{
	...
	for (display = displays; display; display = display->d_next)
		for (cv = D_cvlist; cv; cv = cv->c_next) {
			if (!cv->c_layer
			    || cv->c_layer->l_layfn != &WListLf)
				continue;
			wlistdata =
			    (struct wlistdata *) cv->c_layer->l_data;
			if (wlistdata->order != WLIST_MRU)
				continue;
...
void display_bindkey(title, tab)
char *title;
struct action *tab;
{
	...
	bindkeydata->page = 1;
	bindkeydata->pages =
	    (n + flayer->l_height - 6) / (flayer->l_height - 5);
...
static void bindkeypage()
{
	...
	sprintf(tbuf, "%s key bindings, page %d of %d.",
		bindkeydata->title, bindkeydata->page, bindkeydata->pages);
	centerline(tbuf, 0);
	y = 2;
	for (i = bindkeydata->pos;
	     i < KMAP_KEYS + KMAP_AKEYS + kmap_extn
	     && y < flayer->l_height - 3; i++) {
		p = tbuf;
		xch = "   ";
		if (i < KMAP_KEYS) {
	...
	sprintf(tbuf, "[Press Space %s Return to end.]",
		bindkeydata->page <
		bindkeydata->pages ? "for next page;" : "or");
	centerline(tbuf, flayer->l_height - 2);
	LaySetCursor();
}

static void BindkeyProcess(ppbuf, plen)
char **ppbuf;
int *plen;
{
	int done = 0;
...
static void BindkeyRedisplayLine(y, xs, xe, isblank)
int y, xs, xe, isblank;
{
	if (y < 0) {
		bindkeypage();
...
int n, x, y;
{
	...
	if (l < n)
		LPutStr(flayer, (char *) blank, n - l, &mchar_blank, x + l,
			y);
}
