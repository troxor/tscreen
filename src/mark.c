...
static int nextchar(int *xp, int *yp, int direction, char target, int num)
{
	...
	int step = 1;		/* amount to increment x (+1 or -1) */
...
static void nextword(xp, yp, flags, num)
int *xp, *yp, flags, num;
{
...
char *pt;
{
	...
	for (; i <= y2; i++, ry++) {
		...
		for (to = fore->w_width, im = ml->image + to; to >= 0;
		     to--)
			if (*im-- != ' ')
				break;
		if (i == y2 && x2 < to)
...
#endif
			if (pastefont) {
				c = EncodeChar(pt, c | cf << 16,
					       fore->w_encoding, &font);
				l += c;
				if (pt)
					pt += c;
				continue;
...
{				/* return value 1 if copybuffer changed */
	...
	for (yy = y - 1; yy >= 0; yy--) {
		...
		if (xx < 0 || eq(linep[xx], q)) {	/* line is matching... */
			for (i = fore->w_width - 1, linep += i; i >= x;
			     i--)
				if (*linep-- != ' ')
					break;
			if (i >= x)
	...
	if ((D_user->u_plop.buf =
	     (char *) malloc((unsigned) (i - x + 2))) == NULL) {
...
void MarkRoutine()
{
	...
	LMsg(0, "Copy mode - Column %d Line %d(+%d) (%d,%d)",
	     x + 1, W2D(y + 1), fore->w_histheight, fore->w_width,
	     fore->w_height);
...
static void MarkProcess(inbufp, inlenp)
char **inbufp;
int *inlenp;
{
	char *inbuf, *pt;
...
*/
		...
		case ';':
			...
			if (!rep_cnt)
				rep_cnt = 1;
			nextchar(&cx, &cy, markdata->f_cmd.direction,
		...
		case '+':
			...
			if (j > fore->w_histheight + fore->w_height - 1)
				j = fore->w_histheight + fore->w_height -
				    1;
			revto(linestart(j), j);
			break;
		case '-':
		...
		case 'G':
			...
			if (rep_cnt == 0)
				rep_cnt =
				    fore->w_histheight + fore->w_height;
		...
		case 'H':
			...
			break;
		case 'M':
			revto(markdata->left_mar,
			      D2W((fore->w_height - 1) / 2));
			break;
		case 'L':
		...
		case 'B':
			if (rep_cnt == 0)
				rep_cnt = 1;
			nextword(&cx, &cy,
				 NW_BACK | NW_ENDOFWORD | NW_MUSTMOVE | (od
									 ==
									 'B'
									 ?
									 NW_BIG
		...
		case '\r':
			if (!markdata->second) {
				...
				if (newcopylen > 0) {
					...
					md_user->u_plop.len +=
					    rem(markdata->x1, markdata->y1,
						x2, y2,
						markdata->hist_offset ==
						fore->w_histheight,
						md_user->u_plop.buf +
						md_user->u_plop.len, yend);
#ifdef ENCODINGS
					md_user->u_plop.enc =
					    fore->w_encoding;
#endif
...
int tx, ty, line;
{
	...
	ml = WIN(ty);
	if (ty == fy && fx + 1 == tx && dw_right(ml, tx, fore->w_encoding)
	    && tx < D_width - 1)
		tx++;
	if (ty == fy && fx - 1 == tx && dw_right(ml, fx, fore->w_encoding)
	    && tx)
		tx--;
#endif
...
static void MarkAbort()
{
	...
	if (fore->w_histheight - markdata->hist_offset < fore->w_height) {
		markdata->second = 0;
		yend -=
		    MarkScrollUpDisplay(fore->w_histheight -
					markdata->hist_offset);
	}
	if (markdata->hist_offset != fore->w_histheight) {
		LAY_CALL_UP(LRefreshAll(flayer, 0));
	} else {
		rem(markdata->x1, markdata->y1, markdata->cx, markdata->cy,
		    redisp, (char *) 0, yend);
	}
	ExitOverlayPage();
}
...
int isblank;
{
	...
	if (markdata->second == 0) {
		if (dw_right(ml, xs, fore->w_encoding) && xs > 0)
			xs--;
		if (dw_left(ml, xe, fore->w_encoding)
		    && xe < fore->w_width - 1)
			xe++;
		if (xs == 0 && y > 0 && wy > 0
 ...
 */
static int MarkRewrite(ry, xs, xe, rend, doit)
int ry, xs, xe, doit;
struct mchar *rend;
{
	...
	t = y * fore->w_width + xs;
	for (rm = fore->w_width, i = ml->image + fore->w_width; rm >= 0;
	     rm--)
		if (*i-- != ' ')
			break;
	...
	x = xs;
	while (dx--) {
		if (t >= st && t <= en && x >= markdata->left_mar
		    && x <= rm) {
			if (pastefont)
				mchar_marked.font = ml->font[x];
...
void MakePaster(pa, buf, len, bufiscopy)
struct paster *pa;
char *buf;
int len;
int bufiscopy;
{
	...
	DoProcess(Layer2Window(flayer), &pa->pa_pasteptr, &pa->pa_pastelen,
		  pa);
}
