...
static struct mline *mloff(ml, off)
struct mline *ml;
int off;
{
...
void LScrollH(l, n, y, xs, xe, bce, ol)
struct layer *l;
int n, y, xs, xe;
int bce;
struct mline *ol;
{
	...
	for (cv = l->l_cvlist; cv; cv = cv->c_lnext)
		for (vp = cv->c_vplist; vp; vp = vp->v_next) {
			...
			if (D_blocked)
				continue;
			ScrollH(y2, xs2, xe2, n, bce,
				ol ? mloff(ol, -vp->v_xoff) : 0);
			if (xe2 - xs2 == xe - xs)
				continue;
			if (n > 0) {
				xs2 = xe2 + 1 - n;
	...
	for (cv = l->l_cvlist; cv; cv = cv->c_lnext)
		for (vp = cv->c_vplist; vp; vp = vp->v_next) {
			...
			if (D_blocked)
				continue;
			rol = RECODE_MLINE(ol);
			InsChar(RECODE_MCHAR(c2), xs2, xe2, y2,
				mloff(rol, -vp->v_xoff));
			if (f)
...
void LClearLine(l, y, xs, xe, bce, ol)
struct layer *l;
int xs, xe, bce;
struct mline *ol;
{
	...
	for (cv = l->l_cvlist; cv; cv = cv->c_lnext)
		for (vp = cv->c_vplist; vp; vp = vp->v_next) {
			...
			if (D_blocked)
				continue;
			ClearLine(ol ? mloff(RECODE_MLINE(ol), -vp->v_xoff)
				  : (struct mline *) 0, y2, xs2, xe2, bce);
		}
}

void LClearArea(l, xs, ys, xe, ye, bce, uself)
struct layer *l;
int xs, ys, xe, ye;
int bce;
int uself;
	...
	for (cv = l->l_cvlist; cv; cv = cv->c_lnext) {
		...
		for (vp = cv->c_vplist; vp; vp = vp->v_next) {
			...
			if (ye2 != ye + vp->v_yoff)
				xe2 = xce;
			display = cv->c_display;
			ClearArea(xs2, ys2, xcs, xce, xe2, ye2, bce,
				  uself);
#else
			...
			if (xe == l->l_width - 1 || ye2 != ye + vp->v_yoff)
				xe2 = vp->v_xe;
			display = cv->c_display;
			ClearArea(xs2, ys2, vp->v_xs, vp->v_xe, xe2, ye2,
				  bce, uself);
#endif
		}
	}
...
void LCDisplayLine(l, ml, y, xs, xe, isblank)
struct layer *l;
struct mline *ml;
int y, xs, xe;
int isblank;
	...
	for (cv = l->l_cvlist; cv; cv = cv->c_lnext) {
		...
		for (vp = cv->c_vplist; vp; vp = vp->v_next) {
			...
			if (xs2 > xe2)
				continue;
			display = cv->c_display;
			debug3("LCDisplayLine: DisplayLine %d, %d-%d", y2,
			       xs2, xe2);
			debug1("  mloff = %d\n", -vp->v_xoff);
			DisplayLine(isblank ? &mline_blank : &mline_null,
				    mloff(RECODE_MLINE(ml), -vp->v_xoff),
				    y2, xs2, xe2);
		}
	}
}

void LCDisplayLineWrap(l, ml, y, from, to, isblank)
struct layer *l;
struct mline *ml;
int y, from, to;
int isblank;
...
void LWrapChar(l, c, y, top, bot, ins)
struct layer *l;
struct mchar *c;
int y, top, bot;
int ins;
	...
	if (y != bot) {
		...
		for (cv = l->l_cvlist; cv; cv = cv->c_lnext) {
			...
			if (evp == 0
				...
				WrapChar(RECODE_MCHAR(c),
					 vp->v_xoff + l->l_width, y2,
					 vp->v_xoff, -1,
					 vp->v_xoff + l->l_width - 1, -1,
					 ins);
			}
		}
	} else {
			...
			if (vp) {
				...
				*vpp = vp;
				...
				if (top2 < vp->v_ys)
					top2 = vp->v_ys;
				WrapChar(RECODE_MCHAR(c),
					 vp->v_xoff + l->l_width, bot2,
					 vp->v_xoff, top2,
					 vp->v_xoff + l->l_width - 1, bot2,
					 ins);
			}
		}
	}
...
int InitOverlayPage(datasize, lf, block)
int datasize;
struct LayFuncs *lf;
int block;
{
	...
	if ((newlay =
	     (struct layer *) calloc(1, sizeof(struct layer))) == 0) {
	...
	if (p
		...
		|| (block && flayer->l_next == 0))) {
		if (p->w_savelayer && p->w_savelayer != flayer
		    && p->w_savelayer->l_cvlist == 0)
	...
	if (cv && flayer->l_next == 0 && !block) {
		...
		for (cvpp = &flayer->l_cvlist; (cvp = *cvpp);
		     cvpp = &cvp->c_lnext)
			if (cvp == cv)
				break;
		ASSERT(cvp);
...
void ExitOverlayPage()
{
	...
	if (flayer->l_layfn == &WinLf) {
		...
		if (p->w_blocked && p->w_savelayer
		    && p->w_savelayer != flayer && oldlay->l_cvlist) {
