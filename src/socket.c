...
int FindSocket(fdp, nfoundp, notherp, match)
int *fdp;
int *nfoundp, *notherp;
char *match;
{
	...
	while ((dp = readdir(dirp))) {
		...
		debug("  store it.\n");
		if ((sent =
		     (struct sent *) malloc(sizeof(struct sent))) == 0)
			continue;
		sent->next = 0;
	...
	if (nfound && (lsflag || ngood != 1) && !quietflag) {
		...
		case 1:
			Msg(0,
			    nfound >
			    1 ? "There are several screens on:" :
	...
	if (ndead && !quietflag) {
		char *m = "Remove dead screens with 'tscreen -wipe'.";
		if (wipeflag)
			Msg(0, "%d socket%s wiped out.", nwipe,
			    nwipe > 1 ? "s" : "");
		else
...
int MakeServerSocket()
{
	...
	if ((s = open(SockPath, O_WRONLY | O_NONBLOCK)) >= 0) {
		...
		Msg(0, "There is already a tscreen running on %s.",
		    Filename(SockPath));
		...
		if ((st.st_mode & 0700) == 0600)
			Panic(0, "To resume it, use \"tscreen -r\"");
		else
...
int MakeServerSocket()
{
...
#ifdef SOCK_NOT_IN_FS
	{
		int f;
		if ((f =
		     secopen(SockPath, O_RDWR | O_CREAT, SOCKMODE)) < 0)
...
struct NewWindow *nwin;
{
	...
	if (nwin->aka != nwin_undef.aka
	    && p + strlen(nwin->aka) + 1 <
	    m.m.create.line + sizeof(m.m.create.line))
		strcpy(p, nwin->aka);
	else
		*p = '\0';
	m.m.create.nargs = n;
...
char *tty, *buf;
{
	...
	if (write(s, (char *) &m, sizeof m))
		Panic(errno, "Unable to write to fd %d", s);
	close(s);
	return 0;
}
...
static int CreateTempDisplay(m, recvfd, wi)
struct msg *m;
int recvfd;
struct win *wi;
{
	...
	if (attach) {
		if (display || wi) {
			if (write(i, "Attaching from inside of tscreen?\n", 33))
				Msg(0, "Unable to write to %d", i);
			close(i);
			Kill(pid, SIG_BYE);
		...
		if (strcmp(user, LoginName))
			if (*FindUserPtr(user) == 0) {
				if (write(i, "Access to session denied.\n", 26))
					Msg(0, "Unable to write to %d", i);
				close(i);
				Kill(pid, SIG_BYE);
				Msg(0,
...
#ifndef MULTI
		if (displays) {
			if (write(i, "Screen session in use.\n", 23))
				Msg(0, "Unable to write to %d", i);
			close(i);
			Kill(pid, SIG_BYE);
			return -1;
	...
	GetTTY(i, &Mode);
	if (MakeDisplay
	    (user, m->m_tty, attach ? m->m.attach.envterm : "", i, pid,
	     &Mode) == 0) {
		if (write(i, "Could not make display.\n", 24))
			Msg(0, "Unable to write fd %d", i);
		close(i);
	...
	if (attach) {
		...
		D_encoding =
		    m->m.attach.encoding ==
		    1 ? UTF8 : m->m.attach.encoding ? m->m.attach.
		    encoding - 1 : 0;
#else
		D_encoding =
	...
	if (!display) {
		for (wi = windows; wi; wi = wi->w_next)
			if (!TTYCMP(m.m_tty, wi->w_tty)) {
				...
				display =
				    wi->w_layer.l_cvlist ? wi->w_layer.
...
struct msg *m;
{
	 ...
	 * the old terminal, reread the termcap entries in .tscreenrc
	 * (and nothing more from .tscreenrc is read. Mainly because
	 * I did not check, weather a full reinit is safe. jw)
	 * and /etc/tscreenrc, and initialise anew.
	 */
	if (extra_outcap)
		free(extra_outcap);
	if (extra_incap)
...
#ifdef MULTIUSER
		if (!AclCheckPermCmd
		    (D_user, ACL_EXEC, &comms[RC_WINDOWLIST]))
#endif
		{
	...
	if (displays->d_next == 0 && console_window) {
		if (TtyGrabConsole(console_window->w_ptyfd, 1, "reattach")
		    == 0)
			Msg(0, "console %s is on window %d", HostName,
	...
	if (!dfp) {
		sleep(1);
		debug1
		    ("Attacher %d must not debug, as we have debug off.\n",
		     pid);
...
static void PasswordProcessInput(ibuf, ilen)
char *ibuf;
int ilen;
{
	struct pwdata *pwdata;
	...
	while (ilen-- > 0) {
		...
		if (c == '\r' || c == '\n') {
			...
			pwdata->buf[l] = 0;
			if (strncmp
			    (crypt(pwdata->buf, up), up, strlen(up))) {
				/* uh oh, user failed */
...
struct msg *mp;
{
	...
	*pp = 0;
	...
	if (user->u_password && *user->u_password) {
		Msg(0, "User %s has a password, cannot use -X option.",
		    mp->m.attach.auser);
		return;
	}
#endif
	...
	for (fore = windows; fore; fore = fore->w_next)
		if (!TTYCMP(mp->m_tty, fore->w_tty)) {
			if (!display)
				display =
				    fore->w_layer.l_cvlist ? fore->w_layer.
				    l_cvlist->c_display : 0;
			break;
		}
	if (!display)
	...
	if (*args) {
		...
		if (fore && fore->w_savelayer
		    && (fore->w_blocked
			|| fore->w_savelayer->l_cvlist == 0))
