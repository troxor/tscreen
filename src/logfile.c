...
static int logfile_reopen(name, wantfd, l)
char *name;
int wantfd;
struct logfile *l;
{
	...
	close(wantfd);
	if (((got_fd =
	      open(name, O_WRONLY | O_CREAT | O_APPEND, 0666)) < 0)
...
static struct logfile *lookup_logfile(name)
char *name;
{
	struct logfile *l;
...
struct logfile *logfopen(name, fp)
char *name;
FILE *fp;
{
	struct logfile *l;
...
struct logfile *l;
{
	...
	if (!l)
		for (l = logroot; l; l = l->next) {
			if (stolen_logfile(l)
			    && lf_reopen_fn(l->name, fileno(l->fp), l))
				return -1;
			r |= fflush(l->fp);
			l->flushcount++;
			changed_logfile(l);
