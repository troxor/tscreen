...
#endif
static void disp_history_fn __P((struct event *, char *));
...
extern struct alias *g_aliases_list;
...
void DefRedisplayLine(y, xs, xe, isblank)
int y, xs, xe, isblank;
{
	if (isblank == 0 && y >= 0)
...
void DefClearLine(y, xs, xe, bce)
int y, xs, xe, bce;
{
	LClearLine(flayer, y, xs, xe, bce, (struct mline *) 0);
}

 /*ARGSUSED*/ int DefRewrite(y, xs, xe, rend, doit)
int y, xs, xe, doit;
struct mchar *rend;
{
	return EXPENSIVE;
...
struct LayFuncs BlankLf = {
 ...
 /*ARGSUSED*/ static int BlankResize(wi, he)
int wi, he;
{
	flayer->l_width = wi;
	flayer->l_height = he;
...
struct display *MakeDisplay(uname, utty, term, fd, pid, Mode)
...
struct mode *Mode;
{
...
#ifdef MULTI
	if ((display =
	     (struct display *) calloc(1, sizeof(*display))) == 0)
		return 0;
#else
	if (displays)
		return 0;
	...
	D_blankerev.fd = -1;
#endif
	D_historyev.type = EV_READ;
	D_historyev.data = (char *) display;
	D_historyev.handler = disp_history_fn;
	D_historyev.fd = -1;
	D_OldMode = *Mode;
	D_status_obuffree = -1;
	...
	*dp = display->d_next;
	...
	for (p = windows; p; p = p->w_next) {
		...
		if (p->w_lastdisp == display)
			p->w_lastdisp = 0;
		if (p->w_readev.condneg == &D_status
		    || p->w_readev.condneg == &D_obuflenmax)
...
static void CanvasInitBlank(cv)
struct canvas *cv;
{
...
static struct canvas **CreateCanvasChainRec(cv, cvp)
struct canvas *cv;
struct canvas **cvp;
{
...
struct canvas *cv;
{
	...
	if (cv->c_slback && cv->c_slback->c_slperp == cv)
		cv->c_slback->c_slperp =
		    cv->c_slnext ? cv->c_slnext : cv->c_slprev;
	if (cv->c_slperp) {
		while (cv->c_slperp)
...
struct canvas *cv;
{
	...
	for (; cv; cv = cvn) {
		cvn = cv->c_slnext;
		if (i > maxi) {
			if (cv->c_slprev && !cv->c_slback->c_slback
			    && !cv->c_slprev->c_slperp
...
static struct canvas *AddPerp(cv)
struct canvas *cv;
{
	struct canvas *pcv;
...
void RethinkViewportOffsets(cv)
struct canvas *cv;
{
	struct viewport *vp;
...
int c;
{
	...
	if (D_xtable && D_xtable[(int) (unsigned char) D_rend.font]
	    && D_xtable[(int) (unsigned char) D_rend.
			font][(int) (unsigned char) c])
		AddStr(D_xtable[(int) (unsigned char) D_rend.font]
		       [(int) (unsigned char) c]);
	else
		AddChar(D_rend.font !=
			'0' ? c : D_c0_tab[(int) (unsigned char) c]);
...
static int CallRewrite(y, xs, xe, doit)
int y, xs, xe, doit;
{
	...
	for (cv = D_cvlist; cv; cv = cv->c_next) {
		if (y < cv->c_ys || y > cv->c_ye || xe < cv->c_xs
		    || xs > cv->c_xe)
			continue;
		for (vp = cv->c_vplist; vp; vp = vp->v_next)
			if (y >= vp->v_ys && y <= vp->v_ye
			    && xe >= vp->v_xs && xs <= vp->v_xe)
				break;
		if (vp)
			break;
	}
	if (doit) {
		...
		flayer->l_cvlist = cv;
		cv->c_lnext = 0;
		LayRewrite(y - vp->v_yoff, xs - vp->v_xoff,
			   xe - vp->v_xoff, &D_rend, 1);
	...
	debug3("Calling Rewrite %d %d %d\n", y - vp->v_yoff,
	       xs - vp->v_xoff, xe - vp->v_xoff);
	cost =
	    LayRewrite(y - vp->v_yoff, xs - vp->v_xoff, xe - vp->v_xoff,
		       &D_rend, 0);
	flayer = oldflayer;
	if (D_insert)
...
int x2, y2;
{
	...
	if (x1 >= 0) {		/* relativ x positioning only if we know where we are */
		if (dx > 0) {
			...
			if (dx < costx
			    && (m =
				CallRewrite(y1, x1, x2 - 1, 0)) < costx) {
				costx = m;
				xm = M_RW;
			}
		} else if (dx < 0) {
	...
	if (x2 + D_CRcost < costx
	    && (m =
		(x2 ? CallRewrite(y1, 0, x2 - 1, 0) : 0) + D_CRcost) <
	    costx) {
		costx = m;
		xm = M_CR;
	}
...
void ClearArea(x1, y1, xs, xe, x2, y2, bce, uselayfn)
int x1, y1, xs, xe, x2, y2, bce, uselayfn;
{
	...
	if (x1 == 0 && xs == 0 && (xe == D_width - 1 || y1 == y2)
	    && y1 == 0 && D_CCD && (!bce || D_BE)) {
		GotoPos(x1, y1);
		AddCStr(D_CCD);
		return;
		...
		if (uselayfn) {
			...
			for (cv = D_cvlist; cv; cv = cv->c_next) {
				if (y < cv->c_ys || y > cv->c_ye
				    || xxe < cv->c_xs || x1 > cv->c_xe)
					continue;
				for (vp = cv->c_vplist; vp;
				     vp = vp->v_next)
					if (y >= vp->v_ys && y <= vp->v_ye
					    && xxe >= vp->v_xs
					    && x1 <= vp->v_xe)
			...
			if (cv && cv->c_layer && x1 >= vp->v_xs
			    && xxe <= vp->v_xe && y - vp->v_yoff >= 0
...
void RedisplayDisplays(cur_only)
int cur_only;
{
...
/* XXX: use oml! */
void ScrollH(y, xs, xe, n, bce, oml)
int y, xs, xe, n, bce;
struct mline *oml;
{
	int i;
	...
	if (bce && !D_BE) {
		if (n > 0)
			ClearLine((struct mline *) 0, y, xe - n + 1, xe,
				  bce);
		else
			ClearLine((struct mline *) 0, y, xs, xs - n - 1,
				  bce);
	}
...
void ScrollV(xs, ys, xe, ye, n, bce)
...
*/
	...
	aldlfaster = (n > 1 && ys >= D_top && ye == D_bot
		      && ((up && D_CDL) || (!up && D_CAL)));
...
register int new;
{
	...
	if (D_SA) {
		...
		ospeed = D_dospeed;
		tputs(tparm
		      (D_SA, new & A_SO, new & A_US, new & A_RV,
		       new & A_BL, new & A_DI, new & A_BD, 0, 0, 0), 1,
		      DoAddChar);
		D_rend.attr = new;
		D_atyp = 0;
		if (D_hascolor)
...
int f, b;
{
	...
	if (f != of && f > 15 && D_CCO != 256)
		f = D_CCO == 88
		    && D_CAF ? color256to88(f) : color256to16(f);
	if (f != of && f > 15 && D_CAF) {
		AddCStr2(D_CAF, f);
		of = f;
	}
	if (b != ob && b > 15 && D_CCO != 256)
		b = D_CCO == 88
		    && D_CAB ? color256to88(b) : color256to16(b);
	if (b != ob && b > 15 && D_CAB) {
		AddCStr2(D_CAB, b);
		ob = b;
	}
		...
		for (i = 0; i < 8; i++)
			if (attr2color[i] && (mc->attr & (1 << i)) != 0) {
				if (mc->color == 0 && attr2color[i][3])
					ApplyAttrColor(attr2color[i][3],
						       &mmc);
				else if ((mc->color & 0x0f) == 0
					 && attr2color[i][2])
					ApplyAttrColor(attr2color[i][2],
						       &mmc);
				else if ((mc->color & 0xf0) == 0
					 && attr2color[i][1])
					ApplyAttrColor(attr2color[i][1],
						       &mmc);
				else
					ApplyAttrColor(attr2color[i][0],
						       &mmc);
			}
		mc = &mmc;
		debug2("SetRendition: mapped to %02x %02x\n",
		       (unsigned char) mc->attr,
		       0x99 - (unsigned char) mc->color);
	}
...
char *msg;
{
	 ...
	 */
	if (msg_history[msg_position] != NULL) {
		free(msg_history[msg_position]);
		msg_history[msg_position] = NULL;
	}

	if (((msg_position > 0)
	     && (strcmp(msg, msg_history[msg_position - 1]) != 0)
	     && (msg[0] != '\0')) || ((msg_position == 0) && (msg != NULL)
				      && (msg[0] != '\0'))) {
		msg_history[msg_position] = strdup(msg);
		msg_position += 1;
		if (msg_position >=
		    ((sizeof(msg_history) / sizeof(msg_history[0])) - 1))
			msg_position = 0;
	}
	if (!display)
		return;
	...
	if (D_status) {
		...
		if (!D_status_bell) {
			...
			ti = (now.tv_sec - D_status_time.tv_sec) * 1000 +
			    (now.tv_usec - D_status_time.tv_usec) / 1000;
			if (ti < MsgMinWait)
	...
	*t = '\0';
	...
	D_status_lastx = D_x;
	D_status_lasty = D_y;
	if (!use_hardstatus || D_has_hstatus == HSTATUS_IGNORE
	    || D_has_hstatus == HSTATUS_MESSAGE) {
...
void ShowHStatus(str)
char *str;
{
	int l, ox, oy, max;

	if (D_status == STATUS_ON_WIN && D_has_hstatus == HSTATUS_LASTLINE
	    && STATLINE == D_height - 1)
		return;		/* sorry, in use */
	if (D_blocked)
		return;
	...
	if (D_HS && D_has_hstatus == HSTATUS_HS) {
		...
		if (l > D_width)
			l = D_width;
		GotoPos(0, D_height - 1);
		SetRendition(captionalways || D_cvlist == 0
			     || D_cvlist->
			     c_next ? &mchar_null : &mchar_so);
		PutWinMsg(str, 0, l);
		if (!captionalways && D_cvlist && !D_cvlist->c_next)
			while (l++ < D_width)
				PUTCHARLP(' ');
		if (l < D_width)
			ClearArea(l, D_height - 1, l, D_width - 1,
				  D_width - 1, D_height - 1, 0, 0);
		if (ox != -1 && oy != -1)
			GotoPos(ox, oy);
		D_hstatus = *str ? 1 : 0;
		SetRendition(&mchar_null);
...
void RefreshArea(xs, ys, xe, ye, isblank)
int xs, ys, xe, ye, isblank;
{
	...
	if (!isblank && xs == 0 && xe == D_width - 1 && ye == D_height - 1
	    && (ys == 0 || D_CD)) {
...
void RefreshLine(y, from, to, isblank)
int y, from, to, isblank;
{
	...
	while (from <= to) {
		...
		for (cv = display->d_cvlist; cv; cv = cv->c_next) {
			...
			debug2("  %d %d\n", cv->c_xe, cv->c_ye);
			for (vp = cv->c_vplist; vp; vp = vp->v_next) {
				debug2("  - vp: %d %d", vp->v_xs,
				       vp->v_ys);
				debug2("  %d %d\n", vp->v_xe, vp->v_ye);
				/* find leftmost overlapping vp */
				if (y >= vp->v_ys && y <= vp->v_ye
				    && from <= vp->v_xe && to >= vp->v_xs
				    && (lvp == 0
					|| lvp->v_xs > vp->v_xs)) {
					lcv = cv;
					lvp = vp;
		...
		if (from < lvp->v_xs) {
			if (!isblank)
				DisplayLine(&mline_null, &mline_blank, y,
					    from, lvp->v_xs - 1);
			from = lvp->v_xs;
		}
...
void ClearLine(oml, y, from, to, bce)
struct mline *oml;
int from, to, y, bce;
{
	int x;
...
void DisplayLine(oml, ml, y, from, to)
struct mline *oml, *ml;
int from, to, y;
{
	...
	if (!D_CLP && y == D_bot && to == D_width - 1) {
		if (D_lp_missing || !cmp_mline(oml, ml, to)) {
...
#endif
			{
				...
				delete_lp =
				    !cmp_mchar_mline(&mchar_blank, oml, to)
				    && (D_CE || D_DC || D_CDC);
				D_lp_missing =
				    !cmp_mchar_mline(&mchar_blank, ml, to);
	...
	if (to == D_width - 1 && y < D_height - 1 && D_x == D_width
	    && ml->image[to + 1])
		GotoPos(0, y + 1);
#endif
	if (last2flag) {
...
void WrapChar(c, x, y, xs, ys, xe, ye, ins)
struct mchar *c;
int x, y;
int xs, ys, xe, ye;
int ins;
...
int wi, he;
{
	...
	if (D_width != wi && (D_height == he || !D_CWS) && D_CZ0
...
void ChangeScrollRegion(newtop, newbot)
int newtop, newbot;
{
	if (display == 0)
		return;
	...
	if (select(FD_SETSIZE, &r, (fd_set *) 0, (fd_set *) 0, &t) > 0) {
		...
		if (eat)
			if (read(D_userfd, &buf, 1) == -1)
				Panic(errno, "Cannot read from fd %d", D_userfd);
	}
...
char *data;
{
	...
	/* Hmmmm... a bit ugly... */
	if (D_forecv)
		for (cv = D_forecv->c_layer->l_cvlist; cv;
		     cv = cv->c_lnext) {
	...
	if (D_fore == 0)
		...
		if (W_UWP(D_fore))
			size =
			    sizeof(D_fore->w_pwin->p_inbuf) -
			    D_fore->w_pwin->p_inlen;
		else
#endif
...
{
	display = (struct display *) data;
	debug1("disp_status_fn for display %p\n", display);
	if (D_status)
		RemoveStatus();
}
...
#ifdef MAPKEYS
static void disp_map_fn(ev, data)
struct event *ev;
char *data;
{
	char *p;
...
static void disp_idle_fn(ev, data)
struct event *ev;
char *data;
{
	struct display *olddisplay;
...
/**
 * Dump the recent message history to the console.
 */
void ShowHistoryDump()
{
	int max = (sizeof(msg_history) / sizeof(msg_history[0]));
	int cur = 0;
	int i = msg_position;
	...
	/* start at the prev message */
	i = msg_position - 1;

	if (msg_position < 0)
		msg_position = max - 1;

	for (cur = 0; cur < max; cur += 1) {
		if ((msg_history[i] != NULL)
		    && (msg_history[i][0] != '\0'))
			printf("%s\n", msg_history[i]);
...
/**
 * Open a new window with to display the history.
 */
void ShowHistory(type)
char *type;
{
	char *m;
	int pid;
...
static void disp_history_fn(ev, data)
struct event *ev;
char *data;
{
	char buf[IOSIZE], *b;
	...
	display = (struct display *) data;
	size = read(D_historyev.fd, buf, IOSIZE);
	if (size <= 0) {
		evdeq(&D_historyev);
		close(D_historyev.fd);
		D_historyev.fd = -1;
...
static void disp_blanker_fn(ev, data)
struct event *ev;
char *data;
{
	char buf[IOSIZE], *b;
...
static void DupLayoutCv(cvf, cvt, save)
...
int save;
{
	while (cvf) {
		...
		if (!save) {
			...
			struct win *p =
			    cvf->c_layer ? Layer2Window(cvf->c_layer) : 0;
			cvt->c_layer = p ? &p->w_layer : 0;
		}
		if (cvf->c_slperp) {
			cvt->c_slperp =
			    (struct canvas *) calloc(1,
						     sizeof(struct
							    canvas));
...
struct layout *CreateLayout(title, startat)
char *title;
int startat;
{
	struct layout *lay;
...
static char *AddLayoutsInfo(buf, len, where)
char *buf;
int len;
int where;
{
