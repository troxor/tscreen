...
static int recode_char(c, to_utf, font)
int c, to_utf, font;
{
	int f;
	unsigned short (*p)[2];
	...
	if (font >= 32) {
		...
		if (p)
			for (; (*p)[1]; p++) {
				if ((p[0][0] & 0x8000) && c <= p[0][1]
				    && c >= p[-1][1])
					return (c - p[-1][1] +
						p[-1][0]) | (font & 128 ? 0
							     : font << 8);
				if ((*p)[1] == c)
					return (*p)[0] | (font & 128 ? 0 :
							  font << 8);
			}
	}
	return -1;
...
#ifdef DW_CHARS
static int recode_char_dw(c, c2p, to_utf, font)
...
static int recode_char_to_encoding(c, encoding)
int c, encoding;
{
	char *fp;
	int x;
	...
	if (encodings[encoding].deffont)
		if ((x =
		     recode_char(c, 0, encodings[encoding].deffont)) != -1)
			return x;
	return recode_char(c, 0, -1);
}

#ifdef DW_CHARS
static int recode_char_dw_to_encoding(c, c2p, encoding)
int c, *c2p, encoding;
{
	char *fp;
	int x;
	...
	if ((fp = encodings[encoding].fontlist) != 0)
		while (*fp)
			if ((x =
			     recode_char_dw(c, c2p, 0,
					    (unsigned char) *fp++)) != -1)
				return x;
	if (encodings[encoding].deffont)
		if ((x =
		     recode_char_dw(c, c2p, 0,
				    encodings[encoding].deffont)) != -1)
			return x;
...
struct mchar *recode_mchar(mc, from, to)
struct mchar *mc;
int from, to;
{
...
struct mline *recode_mline(ml, w, from, to)
struct mline *ml;
int w;
int from, to;
{
...
#ifdef DW_CHARS
		if ((from != UTF8 && (c & 0x1f00) != 0
		     && (c & 0xe000) == 0) || (from == UTF8
					       && utf8_isdouble(c))) {
			if (i + 1 == w)
				c = '?';
			else {
...
int c;
{
	...
	if (c >= 0xd800 && c < 0xe000 && combchars
	    && combchars[c - 0xd800]) {
...
int c;
{
	...
	if (c >= 0xd800 && c < 0xe000 && combchars
		...
		return l + ToUtf8(p ? p + l : 0,
				  combchars[c - 0xd800]->c2);
	}
	return ToUtf8(p, c);
}
...
void WinSwitchEncoding(p, encoding)
struct win *p;
int encoding;
{
	int i, j, c;
	...
	for (j = 0; j < p->w_height + p->w_histheight; j++) {
		ml = j <
		    p->w_height ? &p->w_mlines[j] : &p->w_hlines[j -
								 p->
								 w_height];
		if (ml->font == null
		    && encodings[p->w_encoding].deffont == 0)
			continue;
		for (i = 0; i < p->w_width; i++) {
			...
			if (ml->font == null) {
				if ((ml->font =
				     (unsigned char *) calloc(p->w_width +
							      1,
							      1)) == 0) {
					ml->font = null;
					break;
...
#ifdef DW_CHARS
			if ((p->w_encoding != UTF8 && (c & 0x1f00) != 0
			     && (c & 0xe000) == 0)
			    || (p->w_encoding == UTF8
				&& utf8_isdouble(c))) {
				if (i + 1 == p->w_width)
					...
					c2 = ml->image[i] | (ml->
							     font[i] << 8);
					c = recode_char_dw_to_encoding(c,
								       &c2,
								       encoding);
					ml->font[i - 1] = c >> 8 & 255;
					ml->image[i - 1] = c & 255;
...
int c;
{
	...
	return ((c >= 0x1100 && (c <= 0x115f ||	/* Hangul Jamo init. consonants */
		...
		(cjkwidth &&
		 bisearch(c, ambiguous,
			  sizeof(ambiguous) / sizeof(struct interval) -
			  1)));
}
...
int c;
{
	...
	return bisearch(c, combining,
			sizeof(combining) / sizeof(struct interval) - 1);
}

static void comb_tofront(root, i)
int root, i;
{
	for (;;) {
...
struct mchar *mc;
{
	...
	if (!combchars) {
		combchars =
		    (struct combchar **) calloc(0x802,
						sizeof(struct combchar *));
		if (!combchars)
			return;
		combchars[0x800] =
		    (struct combchar *) malloc(sizeof(struct combchar));
		combchars[0x801] =
		    (struct combchar *) malloc(sizeof(struct combchar));
...
#endif
	for (encoding = 0;
	     encoding < (int) (sizeof(encodings) / sizeof(*encodings));
	     encoding++)
...
int EncodingDefFont(encoding)
int encoding;
{
	return encodings[encoding].deffont;
}

void ResetEncoding(p)
struct win *p;
{
	char *c;
...
int DecodeChar(c, encoding, statep)
int c;
int encoding;
int *statep;
{
	...
	if (encoding == BIG5 || encoding == GBK) {
		...
		*statep = 0;
		c &= 0x7f;
		return c << 8 | t | (encoding ==
				     BIG5 ? 030 << 16 : 031 << 16);
	}
	return c | (encodings[encoding].deffont << 16);
}

int EncodeChar(bp, c, encoding, fontp)
char *bp;
int c;
int encoding;
int *fontp;
	...
	if (encoding == UTF8) {
		if (f) {
			...
			if (is_dw_font(f)) {
				int c2 = c & 0xff;
				c = (c >> 8 & 0xff) | (f << 8);
				c = recode_char_dw_to_encoding(c, &c2,
							       encoding);
			} else
#endif
			{
...
int RecodeBuf(fbuf, flen, fenc, tenc, tbuf)
unsigned char *fbuf;
int flen;
int fenc, tenc;
unsigned char *tbuf;
{
	...
	for (i = j = 0; i < flen; i++) {
		...
		if (c < 0)
			continue;
		j += EncodeChar(tbuf ? (char *) tbuf + j : 0, c, tenc,
				&font);
	}
...
#ifdef UTF8
int ContainsSpecialDeffont(ml, xs, xe, encoding)
struct mline *ml;
int xs, xe;
int encoding;
{
...
int LoadFontTranslation(font, file)
int font;
char *file;
{
	...
	for (;;) {
		...
		if (recodetabs[fo].tab
		    && (recodetabs[fo].flags & RECODETAB_ALLOCED) != 0)
...
void LoadFontTranslationsForEncoding(encoding)
int encoding;
{
	char *c;
	int f;
