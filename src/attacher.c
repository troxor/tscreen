...
static void AttacherSigInt __P(SIGPROTOARG);
#if defined(SIGWINCH) && defined(TIOCGWINSZ)
static void AttacherWinch __P(SIGPROTOARG);
#endif
#ifdef LOCK
static void DoLock __P(SIGPROTOARG);
static void LockTerminal __P((void));
static void LockHup __P(SIGPROTOARG);
static void screen_builtin_lck __P((void));
#endif
#ifdef DEBUG
static void AttacherChld __P(SIGPROTOARG);
#endif
#ifdef MULTIUSER
static void AttachSigCont __P(SIGPROTOARG);
#endif
...
static void AttachSigCont SIGDEFARG {
	debug("SigCont()\n");
	ContinuePlease = 1;
}
#endif
...
int how;
{
		...
		default:
			if (rflag < 3) {
				if (quietflag)
					eexit(10 + n);
				Panic(0,
				      "Type \"tscreen [-d] -r [pid.]tty.host\" to resume one of them.");
			}
			/* NOTREACHED */
		}
	}
	...
	if ((st.st_mode & 0600) != 0600)
		Panic(0, "Socket is in wrong mode (%03o)",
		      (int) st.st_mode);
	 ...
	 */
	if ((dflag || !xflag)
	    && (st.st_mode & 0700) != (dflag ? 0700 : 0600))
		Panic(0, "That screen is %sdetached.",
		      dflag ? "already " : "not ");
	if (dflag && (how == MSG_DETACH || how == MSG_POW_DETACH)) {
		m.m.detach.dpid = getpid();
		strncpy(m.m.detach.duser, LoginName,
			sizeof(m.m.detach.duser) - 1);
	...
	strncpy(m.m.attach.envterm, attach_term,
		sizeof(m.m.attach.envterm) - 1);
	...
	strncpy(m.m.attach.preselect, preselect ? preselect : "",
		sizeof(m.m.attach.preselect) - 1);
	...
	m.m.attach.encoding =
	    nwin_options.encoding > 0 ? nwin_options.encoding + 1 : 0;
...
#ifdef DEBUG
static void AttacherChld SIGDEFARG {
	AttacherPanic = 1;
}
#endif
static void AttacherSigAlarm SIGDEFARG {
#ifdef DEBUG
...
static void AttacherFinitBye SIGDEFARG {
	int ppid;
...
static void AttacherNoDebug SIGDEFARG {
...
static void AttacherWinch SIGDEFARG {
	debug("AttacherWinch()\n");
	SigWinchPlease = 1;
}
#endif
...
static char LockEnd[] = "Welcome back to tscreen !!\n";

static void LockHup SIGDEFARG {
	int ppid = getppid();
	 setgid(real_gid);
#ifdef MULTIUSER
	 setuid(own_uid);
...
} static void LockTerminal()
{
	char *prg;
	int sig, pid;
	void(*sigs[NSIG]) __P(SIGPROTOARG);

	for (sig = 1; sig < NSIG; sig++)
		sigs[sig] =
		    signal(sig, sig == SIGCHLD ? SIG_DFL : SIG_IGN);
	...
	/* reset signals */
	for (sig = 1; sig < NSIG; sig++) {
		if (sigs[sig] != (void(*)__P(SIGPROTOARG)) - 1)
			signal(sig, sigs[sig]);
	}
}				/* LockTerminal */
...
static int PAM_conv
__P((int, const struct pam_message **, struct pam_response **, void *));
...
#define COPY_STRING(s) (s) ? strdup(s) : NULL
	...
	for (replies = 0; replies < num_msg; replies++) {
		...
		case PAM_PROMPT_ECHO_OFF:
			...
			reply[replies].resp =
			    appdata_ptr ? strdup((char *) appdata_ptr) : 0;
			break;
		case PAM_TEXT_INFO:
...
static void screen_builtin_lck()
{
	...
	for (;;) {
		...
		pam_error =
		    pam_start("screen", ppp->pw_name, &PAM_conversation,
			      &pamh);
...
void SendCmdMessage(sty, match, av)
char *sty;
char *match;
char **av;
{
	...
	for (; *av && n < MAXARGS - 1; ++av, ++n) {
		len = strlen(*av) + 1;
		if (p + len >=
		    m.m.command.cmd + sizeof(m.m.command.cmd) - 1)
			break;
		strcpy(p, *av);
		p += len;
	}
	*p = 0;
	...
	m.protocol_revision = MSG_REVISION;
	strncpy(m.m.command.preselect, preselect ? preselect : "",
		sizeof(m.m.command.preselect) - 1);
