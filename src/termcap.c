...
char screenterm[30];		/* new $TERM, usually "screen" */
...
int he;
{
	...
	if ((D_tentry =
	     (char *) malloc(TERMCAP_BUFSIZE +
			     (extra_incap ? strlen(extra_incap) +
			      1 : 0))) == 0) {
		Msg(0, "%s", strnomem);
		return -1;
	}
	 ...
	 */
	if (D_HC) {
		Msg(0, "You can't run tscreen on a hardcopy terminal.");
		return -1;
	}
	if (D_OS) {
		Msg(0,
		    "You can't run tscreen on a terminal that overstrikes.");
		return -1;
	}
	if (!D_CL) {
	...
	if (D_CTF) {
		...
		if (!D_CAF && D_ME
		    && (InStr(D_ME, "\033[m") || InStr(D_ME, "\033[0m"))) {
#ifdef TERMINFO
			D_CAF = "\033[3%p1%dm";
			D_CAB = "\033[4%p1%dm";
#else
		...
		if (D_OP && InStr(D_OP, "\033[39;49m"))
			D_CAX = 1;
		if (D_OP
		    && (InStr(D_OP, "\033[m") || InStr(D_OP, "\033[0m")))
			D_OP = 0;
		/* ISO2022 */
		if ((D_EA && InStr(D_EA, "\033(B"))
		    || (D_AS && InStr(D_AS, "\033(0")))
			D_CG0 = 1;
		if (InStr(D_termname, "xterm")
		    || InStr(D_termname, "rxvt"))
			D_CXT = 1;
	...
	for (i = 0; i < NATTR; i++) {
		...
		D_attrtyp[i] =
		    i == ATTR_SO ? xse : (i == ATTR_US ? xue : xme);
	}
	...
	if (D_CC0)
		for (i = strlen(D_CC0) & ~1; i >= 0; i -= 2)
			D_c0_tab[(int) (unsigned char) D_CC0[i]] =
			    D_CC0[i + 1];
	debug1("ISO2022 = %d\n", D_CG0);
	if (D_PF == 0)
		D_PO = 0;
	...
	if (D_tcs[T_CAPS].str && D_tcs[T_CAPS + 10].str
	    && !strcmp(D_tcs[T_CAPS].str, D_tcs[T_CAPS + 10].str))
		D_tcs[T_CAPS].str = 0;
	/* Some xterm entries set kD to ^?. Nuke it. */
	if (D_tcs[T_NAVIGATE_DELETE].str
	    && !strcmp(D_tcs[T_NAVIGATE_DELETE].str, "\0177"))
		D_tcs[T_NAVIGATE_DELETE].str = 0;
	/* wyse52 entries have kcub1 == kb == ^H. Nuke... */
	if (D_tcs[T_CURSOR + 3].str
	    && !strcmp(D_tcs[T_CURSOR + 3].str, "\008"))
...
int map;
{
	...
	for (;;) {
		...
		if (a2 && a2->nr == RC_ILLEGAL)
			a2 = 0;
		if (a1 && a1->nr == RC_STUFF && a1->args[0]
		    && strcmp(a1->args[0], s) == 0)
			a1 = 0;
		if (a2 && a2->nr == RC_STUFF && a2->args[0]
		    && strcmp(a2->args[0], s) == 0)
			a2 = 0;
		domap |= (a1 || a2);
		if (tab == umtab) {
			tab = dmtab;
...
void CheckEscape()
{
	...
	for (display = displays; display; display = display->d_next) {
		for (i = 0; i < D_nseqs; i += D_kmaps[i + 2] * 2 + 4) {
			nr = (D_kmaps[i] << 8 | D_kmaps[i + 1]) &
			    ~KMAP_NOTIMEOUT;
			if (nr < KMAP_KEYS + KMAP_AKEYS) {
				if (umtab[nr].nr == RC_COMMAND)
					break;
				if (umtab[nr].nr == RC_ILLEGAL
				    && dmtab[nr].nr == RC_COMMAND)
					break;
			} else {
				struct kmap_ext *kme =
				    kmap_exts + nr - (KMAP_KEYS +
						      KMAP_AKEYS);
				if (kme->um.nr == RC_COMMAND)
					break;
				if (kme->um.nr == RC_ILLEGAL
				    && kme->dm.nr == RC_COMMAND)
					break;
			}
		}
	}
...
static int findseq_ge(seq, k, sp)
char *seq;
int k;
unsigned char **sp;
{
...
int k;
{
	...
	if (D_kmaps + D_nseqs > p + 2 * k + 4)
		bcopy((char *) p + 2 * k + 4, (char *) p,
		      (D_kmaps + D_nseqs) - (p + 2 * k + 4));
	D_nseqs -= 2 * k + 4;
	D_seqp = D_kmaps + 3;
	D_seql = 0;
	D_seqh = 0;
...
char *s;
{
	...
	if (tcLineLen + (n = strlen(s)) > 55
	    && Termcaplen < TERMCAP_BUFSIZE - 4 - 1) {
...
int aflag;
{
	...
	do {
		...
		p = Term + 5;
		if (!aflag
		    && strlen(screenterm) + strlen(tname) < MAXSTR - 1) {
	...
	if (found) {
		...
		if (tgetstr("im", &xbp) && tgetstr("ic", &xbp) && displays) {
			Msg(0,
			    "Warning: im and ic set in %s termcap entry",
			    p);
		}
	}
#endif
	...
	for (i = T_CAPS; i < T_ECAPS; i++) {
		...
		case T_STR:
			if (D_tcs[i].str == 0)
				break;
			MakeString(term[i].tcname, buf, sizeof(buf),
				   D_tcs[i].str);
			AddCap(buf);
			break;
		case T_FLG:
...
static char *findcap(cap, tepp, n)
char *cap;
char **tepp;
int n;
{
...
static char *e_tgetstr(cap, tepp)
char *cap;
char **tepp;
{
	char *tep;
