...
__P((const char *name, const char *val, char **args, int *argl,
     int count));
static struct alias *FindAlias __P((const char *name));
static struct alias *FindAliasnr __P((int));
static void DelAlias __P((const char *name, int quiet));
...
char *arg;
{
	...
	ret = stat(expanded, &buf);

	/*
	 * test is to be inverted.
	 */
	if (type[0] == '!') {
		invert = 1;
		type += 1;
   ...
   */
	if (strcmp(type, "-d") == 0) {
		...
		/* executable file ? */
		if ((S_ISREG(buf.st_mode)) && (isExecutable(buf.st_mode)))
			result = 1;
	} else if (strcmp(type, "-z") == 0) {
		int len = strlen(expanded);
		result = (len > 0);
		Msg(0, "strlen(%s) is %d", expanded, len);
	} else {
		Msg(0, "Unknown test type : %s", type);
	}
...
char *do_popen(char *cmd)
{
	...
	/* failed, then return the command */
	if (pipe == NULL)
		return (tmp);
	...
	while (!feof(pipe)) {
		...
		if (fgets(buf, sizeof(buf) - 1, pipe) != NULL) {
			int l, i;

			if (res != NULL) {
				char *t = strdup(res);

				int newsize = strlen(t) + strlen(buf) + 1;

				res = realloc(res, newsize * sizeof(char));
				sprintf(res, "%s%s", t, buf);
				free(t);
			} else {
				res =
				    malloc((strlen(buf) +
...
int doComplexTest(test, op1, op2)
char *test;
char *op1;
char *op2;
{
	...
	if ((strcmp(test, "=") == 0) || (strcmp(test, "==") == 0)) {
		...
		result = (strcmp(op1, op2) != 0);
	} else if (strcmp(test, "-eq") == 0) {
		int a = atoi(op1);
		int b = atoi(op2);
		result = (a == b);
	} else if (strcmp(test, "-ne") == 0) {
		int a = atoi(op1);
		int b = atoi(op2);
		result = (a != b);
	} else {
		Msg(0, "Unknown test '%s'", test);
	}
...
void InitKeytab()
{
	...
	for (i = 0; i < NKMAPDEF; i++) {
		...
		if (kmapdef[i] == 0)
			continue;
		argarr[0] = kmapdef[i];
		SaveAction(dmtab + i + (KMAPDEFSTART - T_CAPS), RC_STUFF,
			   argarr, 0);
	}
	for (i = 0; i < NKMAPADEF; i++) {
		...
		argarr[0] = kmapadef[i];
		SaveAction(dmtab + i +
			   (KMAPADEFSTART - T_CURSOR + KMAP_KEYS),
			   RC_STUFF, argarr, 0);
	}
	for (i = 0; i < NKMAPMDEF; i++) {
		...
		argarr[0] = kmapmdef[i];
		argarr[1] = 0;
		SaveAction(mmtab + i + (KMAPMDEFSTART - T_CAPS), RC_STUFF,
			   argarr, 0);
	}
#endif
...
static struct action *FindKtab(class, create)
char *class;
int create;
{
	...
	if (kp == 0) {
		...
		for (i = 0;
		     i < (int) (sizeof(kp->ktab) / sizeof(*kp->ktab));
		     i++) {
		...
		for (;;) {
			...
			if (*D_seqp != ch) {
				...
				if ((q = D_seqh) != 0) {
					...
					i = q[0] << 8 | q[1];
					i &= ~KMAP_NOTIMEOUT;
					debug1("Mapping former hit #%d - ",
					       i);
					debug2("%d(%s) - ", q[2], q + 3);
					if (StuffKey(i))
						ProcessInput2((char *) q +
							      3, q[2]);
					if (display == 0)
						return;
					l -= q[2];
...
static int CheckArgNum(nr, args)
int nr;
char **args;
{
	int i, n;
	...
	if (comms[nr].flags & ARGS_ORMORE) {
		...
		if (i != n && i != n + 1 && i != n + 2 && i != n + 3) {
			Msg(0, orformat[3], rc_name, comms[nr].name,
			    argss[n], argss[n + 1], argss[n + 2],
			    argss[n + 3], "");
			return -1;
		}
	} else if ((comms[nr].flags & ARGS_PLUS1) &&
		   (comms[nr].flags & ARGS_PLUS2)) {
		if (i != n && i != n + 1 && i != n + 2) {
			Msg(0, orformat[2], rc_name, comms[nr].name,
			    argss[n], argss[n + 1], argss[n + 2], "");
			return -1;
		}
	} else if ((comms[nr].flags & ARGS_PLUS1) &&
		...
		if (i != n && i != n + 2 && i != n + 3) {
			Msg(0, orformat[2], rc_name, comms[nr].name,
			    argss[n], argss[n + 2], argss[n + 3], "");
			return -1;
		}
	} else if (comms[nr].flags & ARGS_PLUS1) {
		if (i != n && i != n + 1) {
			Msg(0, orformat[1], rc_name, comms[nr].name,
			    argss[n], argss[n + 1], n != 0 ? "s" : "");
			return -1;
		}
	} else if (comms[nr].flags & ARGS_PLUS2) {
		if (i != n && i != n + 2) {
		...
		Msg(0, orformat[0], rc_name, comms[nr].name, argss[n],
		    n != 1 ? "s" : "");
		return -1;
	}
	return i;
...
static void StuffFin(buf, len, data)
char *buf;
int len;
char *data;
{
...
struct win *p;
{
	...
	if (p->w_hide) {
		...
		p->w_hide = 1;
		Msg(0, "Window %d (%s) is now hidden", p->w_number,
		    p->w_title);
	}
}


 /*ARGSUSED*/ void DoAction(act, key)
struct action *act;
int key;
{
	int nr = act->nr;
	...
	if ((n & NEED_DISPLAY) && display == 0) {
		Msg(0, "%s: %s: display required", rc_name,
		    comms[nr].name);
		return;
	}
	...
	if ((n & NEED_LAYER) && flayer == 0) {
		Msg(0, "%s: %s: display or window required", rc_name,
		    comms[nr].name);
		return;
	}
	...
	if (display) {
		if (AclCheckPermCmd(D_user, ACL_EXEC, &comms[nr])) {
			Msg(0, "%s: %s: permission denied (user %s)",
			    ...
			    (EffectiveAclUser ? EffectiveAclUser :
			     D_user)->u_name);
			return;
		}
	}
	...
	case RC_DEFAUTONUKE:
		if (ParseOnOff(act, &defautonuke) == 0 && msgok)
			Msg(0, "Default autonuke turned %s",
			    defautonuke ? "on" : "off");
		if (display && *rc_name)
	...
	case RC_OBUFLIMIT:
		if (*args == 0)
			Msg(0, "Limit is %d, current buffer size is %d",
			    D_obufmax, D_obuflen);
	...
	case RC_HARDCOPY:
		{
			...
			if (*args && args[1]) {
				Msg(0, "%s: hardcopy: too many arguments",
				    rc_name);
				break;
			}
			if (fore == 0 && *args == 0)
				Msg(0, "%s: hardcopy: window required",
				    rc_name);
			else
	...
	case RC_KILL:
		{
			...
			if (key >= 0) {
				...
				Input(fore->w_pwin ?
				      ...
				      "Really kill this window [y/n]", 1,
				      INP_RAW, confirm_fn, NULL, RC_KILL);
#else
				Input("Really kill this window [y/n]", 1,
				      INP_RAW, confirm_fn, NULL, RC_KILL);
#endif
				break;
			}
		...
		if (key >= 0) {
			Input
			    ("Really quit and kill all your windows [y/n]",
			     1, INP_RAW, confirm_fn, NULL, RC_QUIT);
			break;
		}
		Finit(0);
		/* NOTREACHED */
	...
	case RC_DEBUG:
		...
		if (!*args) {
			if (dfp)
				Msg(0, "debugging info is written to %s/",
				    DEBUGDIR);
			else
				Msg(0,
				    "debugging is currently off. Use 'debug on' to enable.");
			break;
		}
		if (dfp) {
		...
		if (*args == 0 || strcmp("off", *args))
			Msg(0,
			    "Sorry, tscreen was compiled without -DDEBUG option.");
#endif
		break;
	case RC_UNALIASALL:
		{
	...
	case RC_READREG:
#ifdef ENCODINGS
		i = fore ? fore->
		    w_encoding : display ? display->d_encoding : 0;
		if (args[0] && args[1] && !strcmp(args[0], "-e")) {
			i = FindEncoding(args[1]);
			if (i == -1) {
				Msg(0, "%s: readreg: unknown encoding",
				    rc_name);
				break;
			}
			args += 2;
		 ...
		 */
		if ((s = *args) == NULL) {
			Input("Copy to register:", 1, INP_RAW, copy_reg_fn,
			      NULL, 0);
			break;
		}
		if (*argl != 1) {
		...
		if (args[1]) {
			if (args[2]) {
				Msg(0, "%s: readreg: too many arguments",
				    rc_name);
				break;
			}
			if ((s = ReadFile(args[1], &n))) {
				struct plop *pp =
				    plop_tab + (int) (unsigned char) ch;
	...
	case RC_REGISTER:
		...
		if (args[0] && args[1] && !strcmp(args[0], "-e")) {
			i = FindEncoding(args[1]);
			if (i == -1) {
				Msg(0, "%s: register: unknown encoding",
				    rc_name);
				break;
			}
			args += 2;
	...
	case RC_PROCESS:
		if ((s = *args) == NULL) {
			Input("Process register:", 1, INP_RAW, process_fn,
			      NULL, 0);
			break;
		}
		if (*argl != 1) {
	...
	case RC_STUFF:
		...
		if (args[1]) {
			if (strcmp(s, "-k")) {
				Msg(0, "%s: stuff: invalid option %s",
				    rc_name, s);
				break;
			}
			s = args[1];
	...
	case RC_OTHER:
		if (MoreWindows())
			SwitchWindow(display && D_other
				     && !D_other->
				     w_hide ? D_other->w_number :
				     NextWindow());
		break;
	case RC_META:
		if (user->u_Esc == -1)
	...
	case RC_BREAKTYPE:
		{
			...
			if (*args) {
				...
				if (n < 0
					...
					breaktype = n;
					Msg(0, "breaktype set to (%d) %s",
					    n, types[n]);
				}
			} else
				Msg(0, "breaktype is (%d) %s", breaktype,
				    types[breaktype]);
		}
		break;
	case RC_POW_BREAK:
	...
	case RC_HEIGHT:
		{
			...
			if (*args && args[0][0] == '-') {
				Msg(0, "%s: %s: unknown option %s",
				    rc_name, comms[nr].name, *args);
				break;
			}
			if (nr == RC_HEIGHT) {
				if (!*args) {
...
#define H1height 24
			...
			if (what == 2) {
				...
				if (ResizeDisplay(w, h) == 0) {
					Activate(D_fore ?
						 D_fore->w_norefresh : 0);
					/* autofit */
					ResizeLayer(D_forecv->c_layer,
						    D_forecv->c_xe -
						    D_forecv->c_xs + 1,
						    D_forecv->c_ye -
						    D_forecv->c_ys + 1, 0);
	...
	case RC_FLOW:
		if (*args) {
			if (args[0][0] == 'a') {
				fore->w_flow =
				    (fore->w_flow & FLOW_AUTO) ?
				    FLOW_AUTOFLAG | FLOW_AUTO | FLOW_NOW :
				    FLOW_AUTOFLAG;
			} else {
				if (ParseOnOff(act, &n))
					break;
				fore->w_flow =
				    (fore->w_flow & FLOW_AUTO) | n;
			}
		} else {
			if (fore->w_flow & FLOW_AUTOFLAG)
				fore->w_flow =
				    (fore->w_flow & FLOW_AUTO) | FLOW_NOW;
			else if (fore->w_flow & FLOW_NOW)
				fore->w_flow &= ~FLOW_NOW;
			else
				fore->w_flow =
				    fore->w_flow ? FLOW_AUTOFLAG |
				    FLOW_AUTO | FLOW_NOW : FLOW_AUTOFLAG;
		}
	...
	case RC_MONITOR:
		...
		if (n) {
			...
			if (display)	/* we tell only this user */
				ACLBYTE(fore->w_mon_notify,
					D_user->u_id) |=
				    ACLBIT(D_user->u_id);
			else
				for (i = 0; i < maxusercount; i++)
					ACLBYTE(fore->w_mon_notify, i) |=
					    ACLBIT(i);
#endif
	 ...
	 /*FALLTHROUGH*/ case RC_PASTE:
		{
			 ...
			 */
			if ((s = *args) == NULL) {
				Input("Paste from register:", 1, INP_RAW,
				      ins_reg_fn, NULL, 0);
				break;
			}
					...
					if (enc == -1)
						enc = plop_tab[(int)
								...
								char)
							       ch].enc;
					if (enc !=
					    plop_tab[(int) (unsigned char)
						     ch].enc)
						l += RecodeBuf((unsigned
								char *)
							       plop_tab[(int) (unsigned char) ch].buf, plop_tab[(int) (unsigned char) ch].len, plop_tab[(int) (unsigned char) ch].enc, enc, (unsigned char *) 0);
					else
#endif
						l += plop_tab[(int)
							      (unsigned
...
#endif
			if (args[0] && args[1])
				Msg(0, "%s: writebuf: too many arguments",
				    rc_name);
			else
	...
	case RC_READBUF:
		...
		if (args[0] && args[1] && !strcmp(args[0], "-e")) {
			i = FindEncoding(args[1]);
			if (i == -1) {
				Msg(0, "%s: readbuf: unknown encoding",
				    rc_name);
				break;
			}
			args += 2;
	...
	case RC_IGNORECASE:
		...
		if (msgok)
			Msg(0, "Will %signore case in searches",
			    search_ic ? "" : "not ");
		break;
	case RC_ESCAPE:
		if (*argl == 0)
			...
			SetEscape(user, (int) (unsigned char) args[0][0],
				  (int) (unsigned char) args[0][1]);
		else {
			Msg(0, "%s: two characters required after escape.",
			    rc_name);
			break;
		}
	...
	case RC_DEFESCAPE:
		if (*argl == 0)
			...
			SetEscape(NULL, (int) (unsigned char) args[0][0],
				  (int) (unsigned char) args[0][1]);
		else {
			Msg(0,
	...
	case RC_SHELL:
	case RC_DEFSHELL:
		if (ParseSaveExpandedStr(act, &ShellProg) == 0)
	...
	case RC_LOGFILE:
		if (*args) {
			...
			if (ParseSaveExpandedStr(act, &screenlogfile)
			    || !msgok)
				break;
		}
	...
	case RC_TERMINFO:
		if (!rc_name || !*rc_name)
			Msg(0,
			    "Sorry, too late now. Place that in your .tscreenrc file.");
		break;
	case RC_SLEEP:
	...
	case RC_TERM:
		...
		if (ParseSaveStr(act, &s))
			break;
		if (strlen(s) >= 30) {
			Msg(0, "%s: term: argument too long ( < 30)",
			    rc_name);
			free(s);
			break;
		}
	...
	case RC_ECHO:
		...
		if (s)
			...
			Msg(0, "%s: 'echo [-n] [-p] \"string\"' expected.",
			    rc_name);
		break;
	case RC_BELL:
	case RC_BELL_MSG:
	...
	case RC_DEFFLOW:
		if (args[0] && args[1] && args[1][0] == 'i') {
			...
			for (display = displays; display;
				...
				D_NewMode.tio.c_cc[VINTR] =
				    D_OldMode.tio.c_cc[VINTR];
				D_NewMode.tio.c_lflag |= ISIG;
#else				/* TERMIO || POSIX */
				D_NewMode.m_tchars.t_intrc =
				    D_OldMode.m_tchars.t_intrc;
#endif				/* TERMIO || POSIX */
	...
	case RC_DEFSILENCE:
		if (ParseOnOff(act, &n) == 0)
			nwin_default.silence =
			    (n == 0) ? SILENCE_OFF : SILENCE_ON;
		break;
	case RC_VERBOSE:
		if (!*args)
	...
	case RC_HARDSTATUS:
		...
		if (args[0] && strcmp(args[0], "on")
			...
			if (!strcmp(s, "lastline"))
				...
				if (!args[1]) {
					char buf[256];
					AddXChars(buf, sizeof(buf),
						  hstatusstring);
					Msg(0, "hardstatus string is '%s'",
					    buf);
					break;
				}
			} else {
			...
			if (new_use != -1) {
				...
				for (display = displays; display;
					...
					if ((new_use == HSTATUS_LASTLINE
					     && old_use !=
					     HSTATUS_LASTLINE)
					    || (new_use != HSTATUS_LASTLINE
						&& old_use ==
						HSTATUS_LASTLINE))
						ChangeScreenSize(D_width,
								 D_height,
								 1);
					RefreshHStatus();
				}
		...
		if (msgok)
			Msg(0, "messages displayed on %s",
			    use_hardstatus ? "hardstatus line" : "window");
		break;
	case RC_CAPTION:
		if (strcmp(args[0], "always") == 0
		    || strcmp(args[0], "splitonly") == 0) {
			struct display *olddisplay = display;
	...
	case RC_CONSOLE:
		...
		if (TtyGrabConsole(fore->w_ptyfd, n, rc_name))
			break;
		if (n == 0)
			Msg(0, "%s: releasing console %s", rc_name,
			    HostName);
		else if (console_window)
			Msg(0,
			    "%s: stealing console %s from window %d (%s)",
			    rc_name, HostName, console_window->w_number,
			    console_window->w_title);
		else
			Msg(0, "%s: grabbing console %s", rc_name,
			    HostName);
	...
	case RC_ALIAS:
		if (argc == 1)
			...
			AddAlias(args[0], args[1], args + 1, argl + 1,
				 argc - 1);
		break;
	case RC_ALLPARTIAL:
	...
	case RC_VBELLWAIT:
		if (ParseNum1000(act, &VBellWait) == 0 && msgok)
			Msg(0, "vbellwait set to %.10g seconds",
			    VBellWait / 1000.);
		break;
	case RC_MSGWAIT:
		if (ParseNum1000(act, &MsgWait) == 0 && msgok)
			Msg(0, "msgwait set to %.10g seconds",
			    MsgWait / 1000.);
		break;
	case RC_MSGMINWAIT:
		if (ParseNum1000(act, &MsgMinWait) == 0 && msgok)
			Msg(0, "msgminwait set to %.10g seconds",
			    MsgMinWait / 1000.);
		break;
	case RC_SILENCEWAIT:
		...
		if (msgok)
			Msg(0, "silencewait set to %d seconds",
			    SilenceWait);
		break;
	case RC_NUMBER:
		if (*args == 0)
			...
			if (n < 0 || n >= maxwin) {
				Msg(0,
				    "Given window position is invalid.");
				return;
			}
			p = wtab[n];
			wtab[n] = fore;
			...
			if (p && (p->w_slot != (slot_t) - 1)
			    && (p->w_slot != (slot_t) 0)) {
				/* XXX: first display wins? */
				display =
				    fore->w_layer.l_cvlist ? fore->
				    w_layer.l_cvlist->c_display : 0;
				RemoveUtmp(p);
				SetUtmp(p);
			}
#endif
...
#ifdef MULTIUSER
			if (display)	/* we tell only this user */
				ACLBYTE(fore->w_lio_notify,
					D_user->u_id) |=
				    ACLBIT(D_user->u_id);
			else
				for (n = 0; n < maxusercount; n++)
					ACLBYTE(fore->w_lio_notify, n) |=
					    ACLBIT(n);
#endif
			fore->w_silencewait = i;
			fore->w_silence = SILENCE_ON;
			SetTimeout(&fore->w_silenceev,
				   fore->w_silencewait * 1000);
	...
	case RC_SCROLLBACK:
		if (flayer->l_layfn == &MarkLf) {
			Msg(0,
			    "Cannot resize scrollback buffer in copy/scrollback mode.");
			break;
		}
		(void) ParseNum(act, &n);
	...
	case RC_SESSIONNAME:
		if (*args == 0)
			...
			if (ParseSaveStr(act, &s))
				break;
			if (!*s
			    || strlen(s) + (SockName - SockPath) >
			    MAXPATHLEN - 13 || index(s, '/')) {
			...
			sprintf(buf + (SockName - SockPath), "%d.%s",
				(int) getpid(), s);
			free(s);
			if ((access(buf, F_OK) == 0) || (errno != ENOENT)) {
				Msg(0, "%s: inappropriate path: '%s'.",
				    rc_name, buf);
				break;
			}
			if (rename(SockPath, buf)) {
				Msg(errno, "%s: failed to rename(%s, %s)",
				    rc_name, SockPath, buf);
				break;
			}
	...
	case RC_PASTEFONT:
		if (ParseSwitch(act, &pastefont) == 0 && msgok)
			Msg(0, "Will %spaste font settings",
			    pastefont ? "" : "not ");
		break;
	case RC_CRLF:
	...
	case RC_COMPACTHIST:
		if (ParseSwitch(act, &compacthist) == 0 && msgok)
			Msg(0, "%scompacting history lines",
			    compacthist ? "" : "not ");
		break;
	...
	case RC_DEFMODE:
		if (ParseBase(act, *args, &n, 8, "octal"))
			break;
		if (n < 0 || n > 0777) {
			Msg(0, "%s: mode: Invalid tty mode %o", rc_name,
			    n);
			break;
		}
		TtyMode = n;
	...
	case RC_PASSWORD:
		if (*args) {
			...
			if (!fore) {
				Msg(0, "%s: password: window required",
				    rc_name);
				break;
			}
			Input("New tscreen password:", 100, INP_NOECHO,
			      pass1,
	...
	/**
         ...
         * xx -xx bar
         *
         * We can spot these by looking for "-" in the correct place.
         */
			if ((args[0][0] == '-') ||
			    ((strlen(args[0]) >= 2) && (args[0][0] == '!')
			     && (args[0][1] == '-'))) {
				char *type = args[0];
				char *arg = args[1];
				if (doSimpleTest(type, arg)) {
					/* test succeeded - run each remaining arg through eval */
					args = SaveArgs(args);
					for (i = 2; args[i]; i++) {
						if (args[i][0])
							Colonfin(args[i],
	...
	case RC_BINDKEY:
		{
			...
			if (kf == 0) {
				...
				for (i = 0, kme = kmap_exts; i < kmap_extn;
				     i++, kme++)
					if (kme->str == 0) {
						if (args[1])
							break;
	...
	case RC_MAPTIMEOUT:
		if (*args) {
			if (ParseNum(act, &n))
				break;
			if (n < 0) {
				Msg(0, "%s: maptimeout: illegal time %d",
				    rc_name, n);
				break;
			}
			maptimeout = n;
	...
	case RC_BCE:
		if (ParseSwitch(act, &fore->w_bce) == 0 && msgok)
			Msg(0, "Will %serase with background color",
			    fore->w_bce ? "" : "not ");
		break;
#ifdef ENCODINGS
	case RC_KANJI:
	case RC_ENCODING:
		...
		if (*args && !strcmp(args[0], "-d")) {
			if (!args[1])
				Msg(0, "encodings directory is %s",
		...
		if (*args && (!strcmp(args[0], "-l")
			...
			if (msgok)
				Msg(0,
				    "encoding: tscreen is not compiled for UTF-8.");
			break;
		}
#endif
		for (i = 0; i < 2; i++) {
			...
			if (n == -1) {
				Msg(0, "encoding: unknown encoding '%s'",
				    args[i]);
				break;
			}
			if (i == 0 && fore) {
	...
	case RC_DEFENCODING:
		n = FindEncoding(*args);
		if (n == -1) {
			Msg(0, "defencoding: unknown encoding '%s'",
			    *args);
			break;
		}
	...
	case RC_UTF8:
		for (i = 0; i < 2; i++) {
			...
			if (i == 0) {
				...
				if (msgok)
					Msg(0, "Will %suse UTF-8 encoding",
					    n ? "" : "not ");
			} else if (display)
				D_encoding = n ? UTF8 : 0;
			if (args[i] == 0)
	...
	case RC_PRINTCMD:
		...
		if (*args == 0 || msgok) {
			if (printcmd)
				Msg(0, "using '%s' as print command",
				    printcmd);
			else
				Msg(0,
	...
	case RC_CHARSET:
		...
		n = strlen(*args);
		if (n == 0 || n > 6) {
			Msg(0, "%s: %s: string has illegal size.", rc_name,
			    comms[nr].name);
			break;
		}
		if (n > 4 && (((args[0][4] < '0' || args[0][4] > '3')
			       && args[0][4] != '.') || ((args[0][5] < '0'
							  || args[0][5] >
							  '3')
							 && args[0][5]
							 && args[0][5] !=
							 '.'))) {
			Msg(0, "%s: %s: illegal mapping number.", rc_name,
			    comms[nr].name);
			break;
		}
		if (nr == RC_CHARSET) {
	...
	case RC_ATTRCOLOR:
		...
		if (*s || i < 0 || i >= 8) {
			Msg(0, "%s: attrcolor: unknown attribute '%s'.",
			    rc_name, args[0]);
			break;
		}
		n = 0;
		...
		n = 0;
		for (i = 0; i < 8; i++)
			if (attr2color[i][0] || attr2color[i][1]
			    || attr2color[i][2] || attr2color[i][3])
				n |= 1 << i;
		nattr2color = n;
		break;
	case RC_SORENDITION:
		...
		if (msgok)
			Msg(0, "Standout attributes 0x%02x  color 0x%02x",
			    (unsigned char) mchar_so.attr,
	...
	case RC_SU:
		s = NULL;
		if (!*args) {
			Msg(0, "%s:%s tscreen login", HostName, SockPath);
	...
	case RC_FOCUS:
		if (!*args || !strcmp(*args, "down"))
			D_forecv =
			    D_forecv->c_next ? D_forecv->c_next : D_cvlist;
		else if (!strcmp(*args, "up")) {
			struct canvas *cv;
			for (cv = D_cvlist;
			     cv->c_next && cv->c_next != D_forecv;
			     cv = cv->c_next);
			D_forecv = cv;
			...
			Msg(0, "%s: usage: focus [up|down|top|bottom]",
			    rc_name);
			break;
		}
		if ((focusminwidth
	...
	case RC_RESIZE:
		...
		for (; *args; args++) {
			if (!strcmp(*args, "-h"))
				...
				i |= D_forecv->c_slorient ==
				    SLICE_VERT ? RESIZE_FLAG_H :
				    RESIZE_FLAG_V;
		...
		if (*args)
			...
			Input(resizeprompts[i], 20, INP_EVERY, ResizeFin,
			      (char *) 0, i);
		break;
	case RC_SETSID:
	...
	case RC_ALTSCREEN:
		...
		if (msgok)
			Msg(0, "Will %sdo alternate screen switching",
			    use_altscreen ? "" : "not ");
		break;
	case RC_MAXWIN:
		if (ParseNum(act, &n))
	...
	case RC_BACKTICK:
		...
		if (!args[1])
			...
			if (ParseBase
			    (act, args[1], &lifespan, 10, "decimal"))
				break;
	...
	case RC_FOCUSMINSIZE:
		...
		if (msgok) {
			...
			for (i = 0; i < 2; i++) {
				n = i ==
				    0 ? focusminwidth : focusminheight;
				if (n == -1)
					strcpy(b[i], "max");
				else
					sprintf(b[i], "%d", n);
	...
	case RC_LAYOUT:
		if (!strcmp(args[0], "title")) {
			...
			if (!args[1]) {
				Input("Switch to layout: ", 20, INP_COOKED,
				      SelectLayoutFin, NULL, 0);
				break;
			}
			SelectLayoutFin(args[1], strlen(args[1]),
					(char *) 0);
			...
			if (args[1]) {
				lay =
				    layouts ? FindLayout(args[1]) : (struct
								     layout
								     *) 0;
				if (!lay) {
					Msg(0, "unknown layout '%s'",
...
static int DoAlias(name, args, argl)
const char *name;
char **args;
int *argl;
{
...
static void SaveAction(act, nr, args, argl)
struct action *act;
int nr;
char **args;
int *argl;
{
	...
	if ((pp =
	     (char **) malloc((unsigned) (argc + 1) * sizeof(char **))) ==
	    0)
		Panic(0, "%s", strnomem);
...
int *var;
{
	...
	if (num < 0) {
		Msg(0, "%s: %s: invalid argument. Give 'on' or 'off'",
...
int ParseSaveExpandedStr(act, var)
struct action *act;
char **var;
{
	char **args = act->args;
	...
	if (*var)
		free(*var);
	*var = expand_tilde(*args);
	return 0;
}

int ParseNum(act, var)
struct action *act;
int *var;
{
	int i;
...
static int ParseNum1000(act, var)
struct action *act;
int *var;
{
	int i;
...
static struct win *WindowByName(s)
char *s;
{
	struct win *p;
...
static int ParseWinNum(act, var)
struct action *act;
int *var;
{
	char **args = act->args;
...
static int ParseBase(act, p, var, base, bname)
struct action *act;
char *p;
int *var;
int base;
char *bname;
{
	...
	if (*p == 0) {
		Msg(0, "%s: %s: empty argument.", rc_name,
		    comms[act->nr].name);
		return -1;
	}
	while ((c = *p++)) {
		...
		c -= '0';
		if (c < 0 || c >= base) {
			Msg(0, "%s: %s: argument is not %s.", rc_name,
			    comms[act->nr].name, bname);
			return -1;
		}
		i = base * i + c;
	}
...
struct win *wi;
{
...
#endif
			if (p->w_silence) {
				SetTimeout(&p->w_silenceev,
					   p->w_silencewait * 1000);
				evenq(&p->w_silenceev);
			}
			D_other = fore;
			D_fore = 0;
	...
	if (wi && wi->w_type != W_TYPE_GROUP) {
		l = &wi->w_layer;
		if (wi->w_savelayer
		    && (wi->w_blocked || wi->w_savelayer->l_cvlist == 0))
			l = wi->w_savelayer;
	} else {
		l = &cv->c_blank;
		if (wi)
...
int norefresh;
{
	...
	if (MayResizeLayer(D_forecv->c_layer))
		ResizeLayer(D_forecv->c_layer,
			    D_forecv->c_xe - D_forecv->c_xs + 1,
			    D_forecv->c_ye - D_forecv->c_ys + 1, display);
	...
	if (fore) {
		...
		if (ResizeDisplay(fore->w_width, fore->w_height)) {
			debug2("Cannot resize from (%d,%d)", D_width,
			       D_height);
...
static int MoreWindows()
{
	...
	for (pp = wtab; pp != wtab + MAXWIN; pp++) {
		p = *pp;
		if (fore && windows && p && p->w_number != n
		    && p->w_hide == 0)
			return 1;
	}
...
static void UpdateLayoutCanvas(cv, wi)
struct canvas *cv;
struct win *wi;
{
...
char *AddWindows(buf, len, flags, where, attr)
char *buf;
int len;
int flags;
int where;
int attr;
{
	...
	s = ss = buf;
	for (pp = ((flags & 4) && where >= 0) ? wtab + where + 1 : wtab;
	     pp < wtab + MAXWIN; pp++) {
		...
		if ((((flags & 4) && where >= 0 && where < p->w_number) ||
		     (!(flags & 4) && where >= 0 && p->w_number < where))
		    && (flags & 8) && attr != -1)
			a = 1;
		else
			a = 0;
		if (a
		    && (p->w_bell == BELL_DONE || p->w_bell == BELL_FOUND))
			AddWinMsgRend(s, attr);
...
#ifdef MULTIUSER
	    && (ACLBYTE(p->w_mon_notify, D_user->u_id) &
		ACLBIT(D_user->u_id))
#endif
	    )
		*s++ = '@';
...
int where;
{
	...
	ss = AddWindows(buf, sizeof(buf), 0, where, -1);
	s = buf + strlen(buf);
	if (ss - buf > D_width / 2) {
		ss -= D_width / 2;
		if (s - ss < D_width) {
...
static void ShowInfo()
{
	...
	if (wp == 0) {
		Msg(0, "(%d,%d)/(%d,%d) no window", D_x + 1, D_y + 1,
		    D_width, D_height);
		return;
	}
	p = buf;
...
#ifdef MULTIUSER
	    && (ACLBYTE(wp->w_mon_notify, D_user->u_id) &
		ACLBIT(D_user->u_id))
#endif
	    )
...
#ifdef ENCODINGS
	if (wp->w_encoding
	    && (display == 0 || D_encoding != wp->w_encoding
		|| EncodingDefFont(wp->w_encoding) <= 0)) {
		*p++ = ' ';
...
static void ShowDInfo()
{
	...
	if (display == 0)
		return;
	p = buf;
	sprintf(p, "(%d,%d)", D_width, D_height), p += strlen(p);
#ifdef ENCODINGS
	if (D_encoding) {
		*p++ = ' ';
...
static void AKAfin(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
...
static void InputAKA()
{
	char *s, *ss;
	int n;
	Input("Set window's title to: ", sizeof(fore->w_akabuf) - 1,
	      INP_COOKED, AKAfin, NULL, 0);
	s = fore->w_title;
	if (!s)
		return;
	for (; *s; s++) {
...
/**
 * Find possible completions for the given input string, of lenth len.
 *
 * Return the count of completions.
 *
 * matches will be updated to point to a string of space-separated
 * matches.
 *
 * TODO:  Sort the return value.
 *
 */
int findCompletions(char *input, int len, char **matches)
{
	int count = 0;
	int max = RC_LAST;
	...
	for (i = 0; i < max; i++) {
		if (strncmp(input, comms[i].name, len) == 0) {
			if (result[0] != '\0')
				strcat(result, " ");

			strcat(result, comms[i].name);
			count += 1;
		}
	}
	...
	/* now aliases */
	while (cur != NULL) {
		if (strncmp(input, cur->name, len) == 0) {
			if (result[0] != '\0')
				strcat(result, " ");
	...
	/* need this for the completion to work.  sigh. */
	strcat(result, " \t");

	/* *matches = sortString( result , count); */
	*matches = strdup(result);
	return count;
}


static void Colonfin(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
	...
	if (buf[len] == '\t') {
		...
		if (display &&
		    (captionalways || D_has_hstatus == HSTATUS_LASTLINE
		     || (D_canvas.c_slperp
			 && D_canvas.c_slperp->c_slnext)))
			showmessage = 1;

		/* Find completions. */
		completion_count = findCompletions(buf, len,
						   &completion_results);

		if (completion_count == 1) {
			char *s = completion_results + len;
			len = strlen(completion_results) - len;

			LayProcess(&s, &len);
			free(completion_results);
			return;
		}
...
static void SelectFin(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
...
static void SelectLayoutFin(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
...
static void SetenvFin1(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
...
static void SetenvFin2(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
...
char *arg;
{
	...
	if (arg) {
		...
		Input("Setenv: Enter variable name: ", 30, INP_COOKED,
		      SetenvFin1, NULL, 0);
}
...
static void pow_detach_fn(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
	...
	if (ktab[(int) (unsigned char) *buf].nr != RC_POW_DETACH) {
		if (display)
			if (write(D_userfd, "\007", 1))
				Panic(errno, "Unable to bell to D_userfd");
		Msg(0, "Detach aborted.");
	} else
		Detach(D_POWER);
}

static void copy_reg_fn(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
	...
	if (D_user->u_plop.len) {
		if ((pp->buf =
		     (char *) malloc(D_user->u_plop.len)) == NULL) {
			Msg(0, "%s", strnomem);
			return;
		}
...
#endif
	Msg(0, "Copied %d characters into register %c", D_user->u_plop.len,
	    *buf);
}

static void ins_reg_fn(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
...
static void process_fn(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
...
static void confirm_fn(buf, len, data)
char *buf;
int len;
char *data;
{
...
static void su_fin(buf, len, data)
char *buf;
int len;
char *data;
{
	...
	if (!*i->name)
		Input("Screen User: ", sizeof(i->name) - 1, INP_COOKED,
		      su_fin, (char *) i, 0);
	else if (!*i->pw1)
		Input("User's UNIX Password: ", sizeof(i->pw1) - 1,
		      INP_COOKED | INP_NOECHO, su_fin, (char *) i, 0);
	else if (!*i->pw2)
		Input("User's tScreen Password: ", sizeof(i->pw2) - 1,
		      INP_COOKED | INP_NOECHO, su_fin, (char *) i, 0);
	else {
...
static void pass1(buf, len, data)
char *buf;
int len;
char *data;
{
...
static void pass2(buf, len, data)
char *buf;
int len;
char *data;
{
...
static void digraph_fn(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
	...
	if (len >= 1 && ((*buf == 'U' && buf[1] == '+')
		...
		for (i = 0;
		     i < (int) (sizeof(digraphs) / sizeof(*digraphs)); i++)
			if ((digraphs[i][0] == (unsigned char) buf[0]
			     && digraphs[i][1] == (unsigned char) buf[1])
			    || (digraphs[i][0] == (unsigned char) buf[1]
				&& digraphs[i][1] ==
				(unsigned char) buf[0]))
				break;
...
int i;
{
	...
	if (i >= T_CURSOR - T_CAPS && i < T_KEYPAD - T_CAPS
	    && D_cursorkeys)
		i += T_OCAPS - T_CURSOR;
	else if (i >= T_KEYPAD - T_CAPS && i < T_OCAPS - T_CAPS
		 && D_keypad)
		i += T_OCAPS - T_CURSOR;
	debug1(" - action %d\n", i);
	flayer = D_forecv->c_layer;
	...
	if (InMark() || InInput() || InWList())
		act =
		    i <
		    KMAP_KEYS + KMAP_AKEYS ? &mmtab[i] : &kmap_exts[i -
								    (KMAP_KEYS
								     +
								     KMAP_AKEYS)].mm;
	if ((!act || act->nr == RC_ILLEGAL) && !D_mapdefault)
		act =
		    i <
		    KMAP_KEYS + KMAP_AKEYS ? &umtab[i] : &kmap_exts[i -
								    (KMAP_KEYS
								     +
								     KMAP_AKEYS)].um;
	if (!act || act->nr == RC_ILLEGAL)
		act =
		    i <
		    KMAP_KEYS + KMAP_AKEYS ? &dmtab[i] : &kmap_exts[i -
								    (KMAP_KEYS
								     +
								     KMAP_AKEYS)].dm;
...
struct win *FindNiceWindow(wi, presel)
struct win *wi;
char *presel;
{
	int i;
	...
	if (!wi || (IsOnDisplay(wi) && !presel)) {
		...
		if (!wi)
			for (wi = windows; wi; wi = wi->w_next)
				if (wi->w_layer.l_cvlist
				    && !IsOnDisplay(wi)
				    && !AclCheckPermWin(D_user, ACL_WRITE,
							wi))
					break;
		if (!wi)
			for (wi = windows; wi; wi = wi->w_next)
				if (!wi->w_layer.l_cvlist
				    && !AclCheckPermWin(D_user, ACL_READ,
							wi))
					break;
		if (!wi)
			for (wi = windows; wi; wi = wi->w_next)
 ...
 * Add an alias
 */
void AddAlias(name, value, args, argl, count)
const char *name;
const char *value;
char **args;
int *argl;
int count;
{
	...
	/* If we have this alias name already defined then remove it. */
	...
	if ((nr = FindCommnr((char *) value)) == RC_ILLEGAL) {
		...
		if (!alias) {
			Msg(0, "%s: could not find command or alias '%s'",
			    rc_name, value);
			return;
		}
		nr = alias->nr;
 ...
 */
static struct alias *FindAlias(name)
const char *name;
{
	struct alias *t = g_aliases_list;
 ...
 */
static struct alias *FindAliasnr(nr)
int nr;
{
	struct alias *t;
...
void (*hand) __P((struct comm *, char **, int));
{
	...
	if (commtab)
		commtab =
		    (struct commt *) realloc(commtab,
					     sizeof(*commtab) * (ncommtab +
								 n));
	else
		commtab =
		    (struct commt *) malloc(sizeof(*commtab) *
					    (ncommtab + n));
	if (!commtab)
		Panic(0, "%s", strnomem);
	for (i = 0; i < n; i++) {
...
static int CalcSlicePercent(cv, percent)
struct canvas *cv;
int percent;
{
	int w, wsum, up;
...
static int ChangeCanvasSize(fcv, abs, diff, gflag, percent)
...
int percent;
{
	...
	if (abs == 2) {
		if (diff == 0)
			...
			for (cv = fcv->c_slback->c_slperp; cv;
			     cv = cv->c_slnext)
				cv->c_slweight = 0;
			fcv->c_slweight = 1;
			cv = fcv->c_slback->c_slback;
			if (gflag && cv && cv->c_slback)
				ChangeCanvasSize(cv, abs, diff, gflag,
						 percent);
		}
		return diff;
	}
	...
	if (percent) {
		int wsum, up;
		for (cv = fcv->c_slback->c_slperp, wsum = 0; cv;
		     cv = cv->c_slnext)
			wsum += cv->c_slweight;
		if (wsum) {
			up = gflag ? CalcSlicePercent(fcv->
						      c_slback->c_slback,
						      percent) : percent;
	...
	dir = 1;
	for (cv = fcv->c_slnext; diff > 0;
	     cv = dir > 0 ? cv->c_slnext : cv->c_slprev) {
		if (!cv) {
	...
	if (diff && gflag) {
		...
		if (cv && cv->c_slback)
			    ...
			    ChangeCanvasSize(fcv->c_slback->c_slback, 0,
					     diff, gflag, percent);
	}
	fcv->c_slweight += done;
...
static void ResizeRegions(arg, flags)
char *arg;
int flags;
{
	struct canvas *cv;
	...
	if (*arg == '=') {
		...
		if ((cv->
		     c_slperp->c_slorient ^ (SLICE_HORI ^ SLICE_VERT)) &
		    orient) {
			if (cv->c_slback) {
				cv = cv->c_slback;
				EqualizeCanvas(cv->c_slperp, gflag);
...
static void ResizeFin(buf, len, data)
char *buf;
int len;
char *data;
{
	...
	if (ch == 'h')
		...
		flags ^=
		    D_forecv->c_slorient ==
		    SLICE_VERT ? RESIZE_FLAG_H : RESIZE_FLAG_V;
	else if (ch == 'l')
		flags ^= RESIZE_FLAG_L;
	else
		return;
...
int ParseAttrColor(s1, s2, msgok)
char *s1, *s2;
int msgok;
{
	int i, n;
