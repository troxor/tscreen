...
static void tel_connev_fn(ev, data)
struct event *ev;
char *data;
{
	struct win *p = (struct win *) data;
	if (connect
	    (p->w_ptyfd, (struct sockaddr *) &p->w_telsa,
	     sizeof(p->w_telsa)) && errno != EISCONN) {
		char buf[1024];
		buf[0] = ' ';
...
char **args;
{
	...
	if (setsockopt
	    (fd, SOL_SOCKET, SO_OOBINLINE, (char *) &on, sizeof(on)))
...
struct win *p;
{
	...
	if ((p->w_telsa.sin_addr.s_addr = inet_addr(*args)) == -1) {
		...
		bcopy((char *) hp->h_addr,
		      (char *) &p->w_telsa.sin_addr.s_addr, hp->h_length);
		p->w_telsa.sin_family = hp->h_addrtype;
	}
	p->w_telsa.sin_port = htons(port);
	if (port != TEL_DEFPORT)
		sprintf(buf, "Trying %s %d...",
			inet_ntoa(p->w_telsa.sin_addr), port);
	else
		sprintf(buf, "Trying %s...",
			inet_ntoa(p->w_telsa.sin_addr));
	WriteString(p, buf, strlen(buf));
	if (connect
	    (p->w_ptyfd, (struct sockaddr *) &p->w_telsa,
	     sizeof(p->w_telsa))) {
...
void TelProcessLine(bufpp, lenp)
char **bufpp;
int *lenp;
{
...
static void TelReply(p, str, len)
struct win *p;
char *str;
int len;
{
...
static void TelDocmd(p, cmd, opt)
struct win *p;
int cmd, opt;
{
	unsigned char b[3];
	...
	case TC_DO:
		if (p->w_telmopts[opt])
			return;
		repl = TC_WONT;
		if (opt == TO_TTYPE || opt == TO_SGA || opt == TO_BINARY
		    || opt == TO_NAWS || opt == TO_TM || opt == TO_LFLOW) {
			repl = TC_WILL;
			p->w_telmopts[opt] = 1;
		}
...
struct win *p;
{
	...
	case TO_TTYPE:
		...
		if (l >= 20)
			break;
		sprintf(trepl, "%c%c%c%c%s%c%c", TC_IAC, TC_SB, TO_TTYPE,
			0, screenterm, TC_IAC, TC_SE);
...
struct win *p;
{
	...
	if (p->w_width == 0 || p->w_height == 0 || !p->w_telmopts[TO_NAWS])
		return;
	sprintf(s, "%c%c%c%c%c%c%c%c%c", TC_SB, TC_SB, TO_NAWS,
		p->w_width / 256, p->w_width & 255, p->w_height / 256,
