 ...
 */
void Input(istr, len, mode, finfunc, priv, data)
char *istr;
int len;
int mode;
...
static void erase_chars(inpdata, from, to, x, mv)
struct inpdata *inpdata;
char *from;
char *to;
int x;
int mv;
{
	...
	if (inpdata->inp.len > to - inpdata->inp.buf)
		bcopy(to, from,
		      inpdata->inp.len - (to - inpdata->inp.buf));
	chng = to - from;
	if (mv) {
		x -= chng;
...
static void InpProcess(ppbuf, plen)
char **ppbuf;
int *plen;
{
	int len, x;
	...
	while (len) {
		...
		if (inpdata->inpmode & INP_EVERY) {
			...
			if (ch) {
				display = inpdisplay;
				(*inpdata->inpfinfunc) (inpdata->inp.buf,
							inpdata->inp.len,
							inpdata->priv);
		...
		if (((unsigned char) ch & 0177) >= ' ' && ch != 0177
			...
			if (!(inpdata->inpmode & INP_NOECHO)) {
				...
				x++;
				if (p <
				    inpdata->inp.buf + inpdata->inp.len) {
					while (p <
					       inpdata->inp.buf +
					       inpdata->inp.len) {
						mc.image = *p++;
						LPutChar(flayer, &mc, x++,
							 INPUTLINE);
					}
		...
		} else if ((ch == '\b' || ch == 0177)
			   && inpdata->inp.pos > 0) {
			erase_chars(inpdata, p - 1, p, x, 1);
		} else if (ch == '\025') {	/* CTRL-U */
			x = inpdata->inpstringlen;
			if (inpdata->inp.len
			    && !(inpdata->inpmode & INP_NOECHO)) {
				LClearArea(flayer, x, INPUTLINE,
					   x + inpdata->inp.len - 1,
					   INPUTLINE, 0, 0);
				LGotoPos(flayer, x, INPUTLINE);
			}
			...
			if (inpdata->inp.len
			    && !(inpdata->inpmode & INP_NOECHO))
				LClearArea(flayer, inpdata->inpstringlen,
					   INPUTLINE,
					   inpdata->inpstringlen +
					   inpdata->inp.len - 1, INPUTLINE,
					   0, 0);
...
{
	LAY_CALL_UP(LayRedisplayLine
		    (INPUTLINE, 0, flayer->l_width - 1, 0));
	ExitOverlayPage();
}
...
int y, xs, xe, isblank;
{
	...
	if (v > 0 && q < r) {
		...
		if (l > r - q)
			l = r - q;
		LPutStr(flayer, inpdata->inpstring + q - s, l, &mchar_so,
			q, y);
		q += l;
		v -= l;
	}
	...
	if (!(inpdata->inpmode & INP_NOECHO) && v > 0 && q < r) {
		...
		if (l > r - q)
			l = r - q;
		LPutStr(flayer, inpdata->inp.buf + q - s, l, &mchar_so, q,
			y);
		q += l;
		v -= l;
	}
