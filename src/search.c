...
int dir;
{
	...
	if (dir == 0) {
		...
		Input((dir > 0 ? "/" : "?"), sizeof(markdata->isstr) - 1,
		      INP_COOKED, (dir > 0 ? searchend : backsearchend),
		      NULL, 0);
}

static void searchend(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
...
static void backsearchend(buf, len, data)
char *buf;
int len;
char *data;			/* dummy */
{
...
static int matchword(pattern, y, sx, ex)
char *pattern;
int y, sx, ex;
{
...
static int is_bm(str, l, p, end, dir)
char *str;
int l, p, end, dir;
{
	int tab[256];
 ...
 /*ARGSUSED*/ static void is_process(p, n, data)	/* i-search */
char *p;
int n;
char *data;			/* dummy */
{
	...
	default:
		if (*p < ' '
		    || markdata->isistrl >= (int) sizeof(markdata->isistr)
		    || markdata->isstrl >=
		    (int) sizeof(markdata->isstr) - 1)
			return;
		...
		pos =
		    is_bm(markdata->isstr, markdata->isstrl, pos,
			  ...
			  (markdata->md_window->w_histheight +
			   flayer->l_height), markdata->isdir);
	if (pos >= 0) {
		x = pos % flayer->l_width;
		y = pos / flayer->l_width;
		LAY_CALL_UP
		    (LayRedisplayLine
		     (INPUTLINE, 0, flayer->l_width - 1, 0); revto(x, y);
...
int dir;
{
	...
	markdata->isstartpos =
	    markdata->cx + markdata->cy * flayer->l_width;
