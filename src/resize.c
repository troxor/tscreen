...
void CheckScreenSize(change_flag)
int change_flag;
{
	int wi, he;
...
void ChangeScreenSize(wi, he, change_fore)
int wi, he;
int change_fore;
{
	struct win *p;
...
void ResizeLayersToCanvases()
{
	...
	for (cv = D_cvlist; cv; cv = cv->c_next) {
		...
		if (!MayResizeLayer(l)) {
			...
			debug("doing resize.\n");
			ResizeLayer(l, cv->c_xe - cv->c_xs + 1,
				    cv->c_ye - cv->c_ys + 1, display);
		}
		...
		if (lx + cv->c_xoff < cv->c_xs) {
			...
			if (n < (cv->c_xe - cv->c_xs + 1) / 2)
				n = (cv->c_xe - cv->c_xs + 1) / 2;
			if (cv->c_xoff - n + cv->c_layer->l_width - 1 <
			    cv->c_xe)
				n = cv->c_xoff + cv->c_layer->l_width - 1 -
				    cv->c_xe;
			cv->c_xoff -= n;
			RethinkViewportOffsets(cv);
		}
...
void ResizeLayer(l, wi, he, norefdisp)
struct layer *l;
int wi, he;
struct display *norefdisp;
{
	...
	flayer = l;
	if (p == 0 && flayer->l_next && flayer->l_next->l_next == 0
	    && LayResize(wi, he) == 0) {
	...
	for (display = displays; display; display = display->d_next) {
		...
		for (cv = D_cvlist; cv; cv = cv->c_next)
			if (cv->c_layer == l) {
				...
				RefreshArea(cv->c_xs, cv->c_ys, cv->c_xe,
					    cv->c_ye, 0);
			}
		if (D_kaablamm) {
			kaablamm();
...
static int BcopyMline(mlf, xf, mlt, xt, l, w)
...
int xf, xt, l, w;
{
	...
	if (mlt->color != null)
		bcopy((char *) mlf->color + xf, (char *) mlt->color + xt,
		      l);
#ifdef COLORS256
	...
	if (mlt->colorx != null)
		bcopy((char *) mlf->colorx + xf, (char *) mlt->colorx + xt,
		      l);
#endif
	return r;
}
...
int wi;
{
	...
	mline_old.image =
	    (unsigned char *) xrealloc((char *) mline_old.image, maxwidth);
	mline_old.attr =
	    (unsigned char *) xrealloc((char *) mline_old.attr, maxwidth);
	mline_old.font =
	    (unsigned char *) xrealloc((char *) mline_old.font, maxwidth);
	mline_old.color =
	    (unsigned char *) xrealloc((char *) mline_old.color, maxwidth);
#ifdef COLORS256
	mline_old.colorx =
	    (unsigned char *) xrealloc((char *) mline_old.color, maxwidth);
#endif
	if (!
	    (blank && null && mline_old.image
	     && mline_old.
	     attr IFFONT(&&mline_old.font) IFCOLOR(&&mline_old.color)
	     IFCOLORX(&&mline_old.colorx)))
		Panic(0, "%s", strnomem);
...
int ChangeWindowSize(p, wi, he, hi)
struct win *p;
int wi, he, hi;
{
	...
	debug3(" from (%d,%d)+%d", p->w_width, p->w_height,
	       p->w_histheight);
	debug3(" to(%d,%d)+%d\n", wi, he, hi);
	...
	if (hi) {
		if ((nhlines =
		     (struct mline *) calloc(hi,
					     sizeof(struct mline))) == 0) {
			Msg(0,
	...
	if (p->w_width == wi) {
		...
		shift = -ncy;
		for (yy = p->w_y + p->w_histheight - 1;
		     yy >= 0 && ncy + shift < he; yy--) {
			ml = OLDWIN(yy);
	...
	while (fy >= 0 && ty >= 0) {
		...
		while (l > 0 && fy >= 0 && ty >= 0) {
			...
			if (p->w_autoaka > 0
			    && fy == p->w_autoaka - 1 + p->w_histheight
			    && lf - lx <= 0)
...
#ifdef TIOCSWINSZ
	if (wi && (p->w_width != wi || p->w_height != he)
	    && p->w_width != 0 && p->w_height != 0 && p->w_ptyfd >= 0
	    && p->w_pid) {
		glwz.ws_col = wi;
		glwz.ws_row = he;
...
struct win *p;
{
	...
	p->w_mlines = ml;
	t = p->w_alt_width;
	p->w_alt_width = p->w_width;
	p->w_width = t;
	t = p->w_alt_height;
	p->w_alt_height = p->w_height;
	p->w_height = t;
...
void EnterAltScreen(p)
struct win *p;
{
	int ox = p->w_x, oy = p->w_y;
	FreeAltScreen(p);
	SwapAltScreen(p);
	ChangeWindowSize(p, p->w_alt_width, p->w_alt_height,
			 p->w_alt_histheight);
	p->w_x = ox;
	p->w_y = oy;
}

void LeaveAltScreen(p)
struct win *p;
{
	if (!p->w_alt_mlines)
		return;
