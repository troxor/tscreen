...
#include <sys/types.h>
#if !defined(B43) && !defined(ISC) && !defined(pyr) && !defined(_CX_UX)
#include <time.h>
#endif
#include <sys/time.h>
...
void sched()
{
	...
	for (;;) {
		...
		if (timeoutev) {
			...
			/* tp - timeout */
			timeout.tv_sec =
			    timeoutev->timeout.tv_sec - timeout.tv_sec;
			timeout.tv_usec =
			    timeoutev->timeout.tv_usec - timeout.tv_usec;
		...
		if (timeoutev)
			debug2(" timeout %d secs %d usecs", (int)timeout.tv_sec,
			       (int)timeout.tv_usec);
		debug(":\n");
		...
		nsel =
		    select(FD_SETSIZE, &r, &w, (fd_set *) 0,
			   timeoutev ? &timeout : (struct timeval *) 0);
		if (nsel < 0) {
			if (errno != EINTR) {
				Panic(errno, "select");
			}
