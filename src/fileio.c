...
register char *str1, *str2;
{
	...
	if (str2) {
		...
		if ((cp =
		     realloc(str2,
			     (unsigned) len1 + len2 + add_colon + 1)) ==
		    NULL)
			Panic(0, "%s", strnomem);
...
char *rcfile;
{
	...
	if ((p = getenv("TSCREENRC")) != NULL && *p != '\0') {
		debug1("  $TSCREENRC has: '%s'\n", p);
		return SaveStr(p);
	} else {
		debug
		    ("  ...nothing in $TSCREENRC, defaulting $HOME/.tscreenrc\n");
		if (strlen(home) > sizeof(buf) - 12)
			Panic(0, "Rc: home too large");
		sprintf(buf, "%s/.tscreenrc", home);
		return SaveStr(buf);
	}
}
 ...
 * 1) rcfilename = "/etc/tscreenrc"
 * 2) rcfilename = RcFileName
 */
int StartRc(rcfilename, nopanic)
char *rcfilename;
int nopanic;
{
	...
	if (display
	    && (!strncmp(D_termname, "vt", 2)
		|| !strncmp(D_termname, "xterm", 5)))
		extra_incap =
		    CatExtra
...
void FinishRc(rcfilename, execute)
char *rcfilename;
int execute;
{
	char buf[2048];
	 ...
	 */
	if (execute)
		fp = secpopen(rc_name, "r");
	else
		fp = secfopen(rc_name, "r");
...
char *rcfilename;
{
	...
	if (*rcfilename == '~') {
		char *tmp = expand_tilde(rcfilename);
		snprintf(filename, sizeof(filename) - 1, "%s", tmp);
		free(tmp);
	} else {
		snprintf(filename, MAXPATHLEN, "%s", rcfilename);
	}
	...
	/*
	 * OK now "filename" points to the file/dir specified.
	 */
		...
		if (S_ISDIR(buf.st_mode)) {
			...
			if (n < 0) {
				...
				while (i < n) {
					...
					/*
					 * If the file is a file and doesn't start with '.' then
					 * use it.
					 */
					if ((stat(fullpath, &ent) == 0) &&
					    (S_ISREG(ent.st_mode)) &&
					    (namelist[i]->d_name[0] !=
					     '.')) {

						/*
						 * Unless the file ends in "~"
						 */
			...
			rc_recursion++;
			FinishRc(filename, exec);
			rc_recursion--;
		}
	} else {
		Msg(0, "%s: source: not found.", filename);
		return;
	}
	...
	case DUMP_SCROLLBACK:
		if (fn == 0) {
			...
			if (hardcopydir && *hardcopydir
				...
				sprintf(fnbuf, "%s/hardcopy.%d",
					hardcopydir, fore->w_number);
			else
				sprintf(fnbuf, "hardcopy.%d",
					fore->w_number);
			fn = fnbuf;
		}
	...
	if (UserContext() > 0) {
		...
		if (f == NULL) {
			...
			case DUMP_SCROLLBACK:
				...
				if (*mode == 'a') {
					putc('>', f);
					for (j = fore->w_width - 2; j > 0;
					     j--)
						putc('=', f);
					fputs("<\n", f);
				}
				if (dump == DUMP_SCROLLBACK) {
					for (i = 0; i < fore->w_histheight;
					     i++) {
						p = (char *) (WIN(i)->
							      image);
			...
			case DUMP_TERMCAP:
				if ((p =
				     index(MakeTermcap(fore->w_aflag),
					   '=')) != NULL) {
					fputs(++p, f);
					putc('\n', f);
				}
...
/*
 * (Almost) secure open, fopen, and popen
 */

FILE *secfopen(name, mode)
char *name;
char *mode;
{
	FILE *fi;
			...
			if ((user_entry = getpwnam(user)) != 0) {
				mem =
				    (char *) malloc(strlen(input) +
						    strlen(user_entry->
							   pw_dir) + 2);
				sprintf(mem, "%s%s", user_entry->pw_dir,
					slash);
				return mem;
			}
		} else {
