...
static void SigChld __P(SIGPROTOARG);
static void SigInt __P(SIGPROTOARG);
static void CoreDump __P(SIGPROTOARG);
static void FinitHandler __P(SIGPROTOARG);
...
static void showCompiledConfig();
#ifdef DEBUG
...
struct logfile *l;
{
	...
	close(wantfd);
	if (((got_fd =
	      secopen(name, O_WRONLY | O_CREAT | O_APPEND, 0666)) < 0)
...
char **av;
{
	...
	screenlogfile = SaveStr("tscreenlog.%n");
	logtstamp_string =
	...
	while (ac > 0) {
		...
		if (--ac > 0 && *ap == '-') {
			...
			if (ap[1] == '-' && !strcmp(ap, "--version"))
				Panic(0, "tscreen version %s", version);
	...
	}
	{
		char *s;
		if ((s = locale_name())) {
			if (!strncmp(s, "zh_", 3) || !strncmp(s, "ja_", 3)
			    || !strncmp(s, "ko_", 3)) {
				cjkwidth = 1;
			}
		}
	...
	if (nwin_options.aka) {
		...
		if (nwin_options.encoding > 0) {
			...
			if (!newbuf)
				Panic(0, "%s", strnomem);
			newsz =
			    RecodeBuf((unsigned char *) nwin_options.aka,
				      len, nwin_options.encoding, 0,
	...
	nwin = nwin_options;
#ifdef ENCODINGS
	nwin.encoding = nwin_undef.encoding;	/* let tscreenrc overwrite it */
#endif
	if (ac)
		nwin.args = av;
	...
	if (SockMatch && (sockp = index(SockMatch, '/'))) {
		*sockp = 0;
		...
		if (*multi) {
			struct passwd *mppp;
			if ((mppp =
			     getpwnam(multi)) == (struct passwd *) 0)
				Panic(0, "Cannot identify account '%s'.",
				      multi);
	...
	if (multi && !multiattach) {
		if (home && strcmp(home, ppp->pw_dir))
			Panic(0,
			      "$HOME must match passwd entry for multiuser screens.");
	}
#endif
...
#define SET_TTYNAME(fatal) do \
  { \
	...
	if (multi && strlen(multi) > 20)
		Panic(0, "tscreen owner name too long - sorry.");
#endif
	if (strlen(home) > MAXPATHLEN - 25)
		Panic(0, "$HOME too long - sorry.");

	attach_tty = "";
	if (!detached && !lsflag && !cmdflag
	    && !(dflag && !mflag && !rflag && !xflag) && !(!mflag
							   && !SockMatch
							   && sty
							   && !xflag)) {
#ifndef NAMEDPIPE
		...
		if (fl != -1
		    && (fl & (O_RDWR | O_RDONLY | O_WRONLY)) == O_RDWR)
			attach_fd = 0;
#endif
		if (attach_fd == -1) {
			if ((n =
			     secopen(attach_tty, O_RDWR | O_NONBLOCK,
				     0)) < 0)
				Panic(0,
...
#endif
	SockDir = getenv("TSCREENDIR");
	if (SockDir) {
		if (strlen(SockDir) >= MAXPATHLEN - 1)
			Panic(0,
			      "Ridiculously long $TSCREENDIR - try again.");
#ifdef MULTIUSER
		if (multi)
			Panic(0,
			      "No $TSCREENDIR with multi screens, please.");
#endif
	}
#ifdef MULTIUSER
	if (multiattach) {
#ifndef SOCKDIR
		sprintf(SockPath, "%s/.tscreen", multi_home);
		SockDir = SockPath;
#else
		SockDir = SOCKDIR;
...
#ifndef SOCKDIR
		if (SockDir == 0) {
			sprintf(SockPath, "%s/.tscreen", home);
			SockDir = SockPath;
		}
#endif
		if (SockDir) {
			...
			if (lstat(SockDir, &st)) {
				...
				if (!S_ISDIR(st.st_mode))
					Panic(0,
					      ...
					      SockDir);
				if (eff_uid == 0 && real_uid
				    && (int) st.st_uid != eff_uid)
					Panic(0,
			...
			if (access(SockPath, F_OK)) {
				if (mkdir(SockPath, 0700) == -1)
					Panic(errno,
					      "Cannot make directory '%s'",
					      SockPath);
				if (chown(SockPath, real_uid, real_gid) == -1)
					Panic(errno,
	...
	if (multi) {
		if ((int) st.st_uid != multi_uid)
			Panic(0, "%s is not the owner of %s.", multi,
			      SockPath);
	} else
#endif
	{
	...
	*SockName = 0;
	...
	debug2("SockPath: %s  SockMatch: %s\n", SockPath,
	       SockMatch ? SockMatch : "NULL");

#if defined(SYSV) && !defined(ISC)
	if (uname(&utsnam) == -1)
		Panic(errno, "uname");
	strncpy(HostName, utsnam.nodename,
		sizeof(utsnam.nodename) <
		MAXSTR ? sizeof(utsnam.nodename) : MAXSTR - 1);
	HostName[sizeof(utsnam.nodename) <
		 MAXSTR ? sizeof(utsnam.nodename) : MAXSTR - 1] = '\0';
#else
	...
	if (cmdflag) {
		...
		debug("tscreen -r: - is there anybody out there?\n");
...
#endif
		debug
		    ("tscreen -r: backend not responding -- still crying\n");
	} else if (dflag && !mflag) {
		SET_TTYNAME(0);
		Attach(MSG_DETACH);
		Msg(0, "[%s %sdetached.]\n", SockName,
	...
	default:
		if (detached)
			exit(0);
		if (SockMatch)
			sprintf(socknamebuf, "%d.%s", MasterPid,
				SockMatch);
		else
			sprintf(socknamebuf, "%d.%s.%s", MasterPid,
				stripdev(attach_tty), HostName);
	...
	ap = av0 + strlen(av0) - 1;
	while (ap >= av0) {
		if (!strncmp("tscreen", ap, 7)) {
			strncpy(ap, "TSCREEN", 7);	/* name this process "TSCREEN" */
			break;
		}
		ap--;
	}
	...
	if (!detached) {
		if (MakeDisplay
		    (LoginName, attach_tty, attach_term, n, getppid(),
		     &attach_Mode) == 0)
			Panic(0, "Could not alloc display");
		PanicPid = 0;
#ifdef ENCODINGS
		D_encoding =
		    nwin_options.encoding > 0 ? nwin_options.encoding : 0;
		debug1("D_encoding = %d\n", D_encoding);
#endif
	}
	...
	if (SockMatch) {
		...
		sprintf(socknamebuf, "%d.%s.%s", (int) getpid(),
			stripdev(attach_tty), HostName);
	}
	for (ap = socknamebuf; *ap; ap++)
		if (*ap == '/')
...
#endif
		FinishRc(ETCSCREENRC, 0);
#endif
	FinishRc(RcFileName, 0);

	debug2("UID %d  EUID %d\n", (int) getuid(), (int) geteuid());
	if (windows == NULL) {
		debug
		    ("We open one default window, as tscreenrc did not specify one.\n");
...
void WindowDied(p, wstat, wstat_valid)
struct win *p;
#ifdef BSDWAIT
union wait wstat;
#else
...
static void SigChldHandler()
{
	...
	if (stat(SockPath, &st) == -1) {
		...
		if (!RecoverSocket()) {
			debug
			    ("tscreen cannot recover from corrupt Socket, bye\n");
			Finit(1);
		} else
...
static void SigChld SIGDEFARG {
	debug("SigChld()\n");
	GotSigChld = 1;
} void SigHup SIGDEFARG {
	/* Hangup all displays */
...
static void SigInt SIGDEFARG {
	...
	if (fore && displays) {
	 ...
	 InterruptPlease = 1;
#endif
} static void CoreDump SIGDEFARG {
	struct display *disp;
	char buf[80];
	 ...
	 unlink("core");
#ifdef SIGHASARG
	 sprintf(buf, "\r\n[tscreen caught signal %d.%s]\r\n", sigsig,
#else
	 sprintf(buf, "\r\n[tscreen caught a fatal signal.%s]\r\n",
#endif
	...
	for (disp = displays; disp; disp = disp->d_next) {
		...
		if (write(disp->d_userfd, buf, strlen(buf)) == -1)
			Panic(errno, "Unable to write to disp->d_userfd");
...
static void FinitHandler SIGDEFARG {
#ifdef SIGHASARG
...
#define AddStrSock(msg) do { \
	...
	if (D_fore) {
		...
		D_user->u_detachotherwin =
		    D_other ? D_other->w_number : -1;
	}
...
void MakeNewEnv()
{
	...
	if (NewEnv)
		free((char *) NewEnv);
	NewEnv = np =
	    (char **) malloc((unsigned) (op - environ + 7 + 1) *
			     sizeof(char **));
	if (!NewEnv)
		Panic(0, "%s", strnomem);
	sprintf(stybuf, "STY=%s",
		strlen(SockName) <= MAXSTR - 5 ? SockName : "?");
	*np++ = stybuf;		/* NewEnv[0] */
	*np++ = Term;		/* NewEnv[1] */
	np++;			/* room for SHELL */
#ifdef TIOCSWINSZ
...
void Msg(int err, char *fmt, VA_DOTS) {
	VA_LIST(ap)
 ...
 */
void Panic(int err, char *fmt, VA_DOTS) {
	VA_LIST(ap)
	char buf[MAXPATHLEN * 2];
	char *p = buf;
	...
	if (displays == 0 && display == 0) {
		...
		for (display = displays; display;
			...
			SetTTY(D_userfd, &D_OldMode);
			fcntl(D_userfd, F_SETFL, 0);
			if (write(D_userfd, buf, strlen(buf)) == -1)
				Panic(errno, "Unable to write '%s' to disp->d_userfd", buf);
			if (write(D_userfd, "\n", 1) == -1)
				Panic(errno, "Unable to write '\\n' to disp->d_userfd");
			freetty();
			if (D_userpid)
				Kill(D_userpid, SIG_BYE);
		}
...
static void backtick_filter(bt)
struct backtick *bt;
{
	char *p, *q;
	int c;
...
static void backtick_fn(ev, data)
struct event *ev;
char *data;
{
	struct backtick *bt;
...
void setbacktick(num, lifespan, tick, cmdv)
int num;
int lifespan;
int tick;
char **cmdv;
...
static char *runbacktick(bt, tickp, now)
struct backtick *bt;
int *tickp;
time_t now;
{
...
int rec;
{
	...
	for (; *s && (l = winmsg_buf + MAXSTR - 1 - p) > 0; s++, p++) {
		*p = *s;
		...
		case 'h':
			...
			}
			{
				...
				*p = 0;
				...
				MakeWinMsgEv(*s ==
					     'h' ? win->
					     w_hstatus : runbacktick(bt,
								     &oldtick,
								     now.
								     tv_sec),
					     win, '\005', 0,
					     (struct event *) 0, rec + 1);
		...
		case '{':
			{
				...
				for (i = 0; i < 127; i++)
					if (s[i] && s[i] != '}')
						...
						break;
				if (s[i] == '}'
				    && winmsg_numrend < MAX_WINMSG_REND) {
					r = -1;
					rbuf[i] = 0;
					debug1("MakeWinMsg attrcolor %s\n",
					       rbuf);
					if (i != 1 || rbuf[0] != '-')
						r = ParseAttrColor(rbuf,
								   (char *)
								   0, 0);
					if (r != -1 && attrflg) {
						attr = r;
		...
		case '<':
			*p = ' ';
			if (num || zeroflg || plusflg || longflg
			    || (*s != '=')) {
				/* expand all pads */
				if (minusflg) {
					num =
					    (plusflg ? lastpad : padlen) -
					    num;
					if (!plusflg && padlen == 0)
						num = p - winmsg_buf;
					plusflg = 0;
	...
	*p = '\0';
	...
	if (ev && tick) {
		...
		ev->timeout = now;
		debug2("NEW timeout %d %d\n", (int)ev->timeout.tv_sec, tick);
	}
	return winmsg_buf;
}

char *MakeWinMsg(s, win, esc)
char *s;
struct win *win;
int esc;
{
...
static void serv_read_fn(ev, data)
struct event *ev;
char *data;
{
	debug("Knock - knock!\n");
...
static void serv_select_fn(ev, data)
struct event *ev;
char *data;
{
	struct win *p;
	...
	if (InterruptPlease) {
		...
		if (fore && displays) {
			...
			if (write(W_UWP(fore) ? fore->w_pwin->p_ptyfd : fore->w_ptyfd, &ibuf, 1) == -1)
				Panic(errno, "Unable to write '%s' to '%d'", &ibuf, 
	...
	for (p = windows; p; p = p->w_next) {
		if (p->w_bell == BELL_FOUND || p->w_bell == BELL_VISUAL) {
			struct canvas *cv;
			int visual = p->w_bell == BELL_VISUAL
			    || visual_bell;
			p->w_bell = BELL_ON;
			for (display = displays; display;
			     display = display->d_next) {
				for (cv = D_cvlist; cv; cv = cv->c_next)
					if (cv->c_layer->l_bottom ==
					    &p->w_layer)
						break;
				if (cv == 0) {
					p->w_bell = BELL_DONE;
					Msg(0, "%s",
					    MakeWinMsg(BellString, p,
						       '%'));
				} else if (visual && !D_VB
					   && (!D_status
					       || !D_status_bell)) {
	...
	for (display = displays; display; display = display->d_next) {
		...
		for (cv = D_cvlist; cv; cv = cv->c_next) {
			...
			if (lx + cv->c_xoff < cv->c_xs) {
				...
				CV_CALL(cv,
					LayRedisplayLine(-1, -1, -1, 1);
					for (i = 0; i < flayer->l_height;
					     i++) {
					LScrollH(flayer, -n, i, 0,
						 flayer->l_width - 1, 0,
						 0);
				...
				if (n < (cv->c_xe - cv->c_xs + 1) / 2)
					n = (cv->c_xe - cv->c_xs + 1) / 2;
				if (cv->c_xoff - n + cv->c_layer->l_width -
				    1 < cv->c_xe)
					n = cv->c_xoff +
					    cv->c_layer->l_width - 1 -
					    cv->c_xe;
				cv->c_xoff -= n;
				RethinkViewportOffsets(cv);
				if (n > cv->c_layer->l_width)
				...
				CV_CALL(cv,
					...
					for (i = 0; i < flayer->l_height; i++) {
						LScrollH(flayer, n, i, 0,
						 flayer->l_width - 1, 0,
						 0);
						LayRedisplayLine(i, flayer->l_width - n,
							 flayer->l_width - 1, 1);
					}
					if (cv == cv->c_display->d_forecv)
					LaySetCursor(););
	...
	for (display = displays; display; display = display->d_next) {
		if (D_status == STATUS_ON_WIN || D_cvlist == 0
		    || D_cvlist->c_next == 0)
			continue;
		debug1("serv_select_fn: Restore on cv %p\n", D_forecv);
		CV_CALL(D_forecv, LayRestore();
			LaySetCursor());
	}
}

static void logflush_fn(ev, data)
struct event *ev;
char *data;
{
	struct win *p;
...
static void showCompiledConfig()
{
	printf("tscreen version %s\n", version);
	printf("%s\n\n", PACKAGE_URL);

	printf("Compilation Options\n\n");

#ifdef ALLOW_SYSSCREENRC
	printf("+ALLOW_SYSSCREENRC\n");
#else
	printf("-ALLOW_SYSSCREENRC\n");
#endif
#ifdef AUTO_NUKE
	...
	printf("-ISC\n");
#endif
#ifdef LOADAV_AVENRUN
	printf("+LOADAV_AVENRUN\n");
#else
	printf("-LOADAV_AVENRUN\n");
#endif
#ifdef LOADAV_GETLOADAVG
	printf("+LOADAV_GETLOADAVG\n");
#else
	printf("-LOADAV_GETLOADAVG\n");
#endif
#ifdef LOADAV_NUM
	...
	printf("-TTYVTIME\n");
#endif
#ifdef TTY_DISABLE_FNBLOCK
	printf("+TTY_DISABLE_FNBLOCK\n");
#else
	printf("-TTY_DISABLE_FNBLOCK\n");
#endif
#ifdef HAVE_SETENV
	...
	printf("+UTMP_LOGINDEFAULT\n");
#else
	printf("-UTMP_LOGINDEFAULT\n");
#endif
#ifdef UTMP_LOGOUTOK
	printf("+UTMP_LOGOUTOK\n");
#else
	printf("-UTMP_LOGOUTOK\n");
#endif
#ifdef UTMP_USRLIMIT
	printf("+UTMP_USRLIMIT\n");
#else
	printf("-UTMP_USRLIMIT\n");
#endif
