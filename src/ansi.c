...
static void MPutStr
__P((struct win *, char *, int, struct mchar *, int, int));
static void MWrapChar
__P((struct win *, struct mchar *, int, int, int, int));
static void MBceLine __P((struct win *, int, int, int, int));

#define CURR_BCE (curr->w_bce ? rend_getbg(&curr->w_rend) : 0)

void ResetAnsiState(p)
struct win *p;
{
	p->w_state = LIT;
	p->w_StringType = NONE;
...
register int len;
{
	...
	do {
		...
		if (curr->w_state == LIT &&
		    ...
		    c >= ' ' && c != 0x7f &&
		    ((c & 0x80) == 0
		     || ((c >= 0xa0 || !curr->w_c1) && !curr->w_gr))
		    && !curr->w_ss && !curr->w_insert
		    && curr->w_x < cols - 1) {
			...
			currx -= curr->w_x;
			if (currx > 0) {
				MPutStr(curr, imp, currx, &curr->w_rend,
					curr->w_x, curr->w_y);
				LPutStr(&curr->w_layer, imp, currx,
					&curr->w_rend, curr->w_x,
					curr->w_y);
				curr->w_x += currx;
			}
			if (len == 0)
		...
		case STRESC:
			...
			case '\\':
				...
				for (cv = curr->w_layer.l_cvlist; cv;
				     cv = cv->c_lnext) {
			...
			case '9':
				...
				if (curr->w_NumArgs >= 0 && curr->w_NumArgs < MAXARGS)
					if (curr->w_args[curr->w_NumArgs] < 100000000)
						curr->w_args[curr->w_NumArgs] = 10 * curr->w_args[curr->w_NumArgs] + (c - '0');
				}
				break;
			case ';':
			case ':':
		...
		default:
			...
			if (curr->w_encoding == UTF8 && c >= 0x0300
				...
				if (oy < 0)
					oy = 0;
				copy_mline2mchar(&omc, &curr->w_mlines[oy],
						 ox);
				if (omc.image == 0xff && omc.font == 0xff) {
					ox--;
					if (ox >= 0) {
						copy_mline2mchar(&omc,
								 &curr->
								 w_mlines
								 [oy], ox);
						omc.mbcs = 0xff;
				...
				if (ox >= 0) {
					...
					copy_mchar2mline(&omc,
							 &curr->
							 w_mlines[oy], ox);
					LPutChar(&curr->w_layer, &omc, ox,
						 oy);
					LGotoPos(&curr->w_layer, curr->w_x,
						 curr->w_y);
				}
				break;
			}
			...
			if (curr->w_x < cols - 1) {
				if (curr->w_insert) {
					save_mline(&curr->
						   w_mlines[curr->w_y],
						   cols);
					MInsChar(curr, &curr->w_rend,
						 curr->w_x, curr->w_y);
					LInsChar(&curr->w_layer,
						 &curr->w_rend, curr->w_x,
						 curr->w_y, &mline_old);
					curr->w_x++;
				} else {
					MPutChar(curr, &curr->w_rend,
						 curr->w_x, curr->w_y);
					LPutChar(&curr->w_layer,
						 &curr->w_rend, curr->w_x,
						 curr->w_y);
					curr->w_x++;
				}
			} else if (curr->w_x == cols - 1) {
				MPutChar(curr, &curr->w_rend, curr->w_x,
					 curr->w_y);
				LPutChar(&curr->w_layer, &curr->w_rend,
					 curr->w_x, curr->w_y);
				if (curr->w_wrap)
					curr->w_x++;
			} else {
				MWrapChar(curr, &curr->w_rend, curr->w_y,
					  curr->w_top, curr->w_bot,
					  curr->w_insert);
				LWrapChar(&curr->w_layer, &curr->w_rend,
					  curr->w_y, curr->w_top,
					  curr->w_bot, curr->w_insert);
				if (curr->w_y != curr->w_bot
				    && curr->w_y != curr->w_height - 1)
					curr->w_y++;
				curr->w_x = 1;
			}
			if (curr->w_mbcs) {
			...
			if (curr->w_ss) {
				curr->w_FontL =
				    curr->w_charsets[curr->w_Charset];
				curr->w_FontR =
				    curr->w_charsets[curr->w_CharsetR];
				curr->w_rend.font = curr->w_FontL;
				LSetRendition(&curr->w_layer,
					      &curr->w_rend);
				curr->w_ss = 0;
			}
			break;
...
static void WLogString(p, buf, len)
struct win *p;
char *buf;
int len;
{
...
int c, intermediate;
{
	...
	case 0:
		...
		case '|':
			...
			break;
		case 'N':	/* SS2 */
			if (curr->w_charsets[curr->w_Charset] !=
			    curr->w_charsets[G2]
			    || curr->w_charsets[curr->w_CharsetR] !=
			    curr->w_charsets[G2])
				curr->w_FontR = curr->w_FontL =
				    curr->w_charsets[curr->w_ss = G2];
			else
				curr->w_ss = 0;
			break;
		case 'O':	/* SS3 */
...
int c, intermediate;
{
	...
	case 0:
		...
		case 'x':	/* decreqtparm */
			if (a1 == 0 || a1 == 1)
				Report("\033[%d;1;1;112;112;1;0x", a1 + 2,
				       0);
			break;
		case 'p':	/* obscure code from a 97801 term */
			if (a1 == 6 || a1 == 7) {
				curr->w_curinv = 7 - a1;
				LCursorVisibility(&curr->w_layer,
						  curr->
						  w_curinv ? -1 : curr->
						  w_curvvis);
			}
	...
	case '>':
		switch (c) {
		case 'c':	/* secondary DA (VT220 Device Attributes) */
			if (a1 == 0)
				// changed nversion to 0 for no good reason, are VT220s still relevant?
				Report("\033[>%d;%d;0c", 83, 0);	/* 83 == 'S' */
			break;
		}
		break;
	}
	...
	case OSC:		/* special xterm compatibility hack */
		...
		if (typ == 83) {	/* 83 = 'S' */
			...
			if (windowuser
				...
				fore = curr;
				flayer =
				    fore->w_savelayer ? fore->
				    w_savelayer : &fore->w_layer;
				DoCommand(args, argl);
				EffectiveAclUser = 0;
				fore = 0;
				flayer = 0;
		...
		if (typ == 0 || typ == 1 || typ == 20 || typ == 39
			...
			if (strcmp(curr->w_xtermosc[typ2], p)) {
				strncpy(curr->w_xtermosc[typ2], p,
					sizeof(curr->w_xtermosc[typ2]) -
					1);
				curr->
				    w_xtermosc[typ2][sizeof
						     (curr->
						      w_xtermosc[typ2]) -
						     1] = 0;
...
int n;
{
	...
	LScrollH(&curr->w_layer, -n, y, x, curr->w_width - 1, CURR_BCE,
		 &mline_old);
...
{
	LClearArea(&curr->w_layer, 0, 0, curr->w_width - 1,
		   curr->w_height - 1, CURR_BCE, 1);
...
static void ClearLineRegion(from, to)
int from, to;
{
...
{
	register int j, i = 0, a = curr->w_rend.attr, c =
	    curr->w_rend.color;
#ifdef COLORS256
	...
	do {
		j = curr->w_args[i];
		if ((j == 38 || j == 48) && i + 2 < curr->w_NumArgs
		    && curr->w_args[i + 1] == 5) {
			int jj;
...
#endif
		if (j < 0
		    || j >= (int) (sizeof(rendlist) / sizeof(*rendlist)))
			continue;
		j = rendlist[j];
		if (j & (1 << NATTR))
			a &= j;
...
int l;
{
	...
	for (i = 0; l > 0; l--) {
		if (p->w_akachange + i ==
		    p->w_akabuf + sizeof(p->w_akabuf) - 1)
			break;
		c = (unsigned char) *s++;
		if (c == 0)
			break;
...
static void FindAKA()
{
	...
	y = (wp->w_autoaka > 0
	     && wp->w_autoaka <=
	     wp->w_height) ? wp->w_autoaka - 1 : wp->w_y;
	cols = wp->w_width;
      try_line:
...
int n1, n2;
{
	...
	if (W_UWP(curr)) {
		if ((unsigned) (curr->w_pwin->p_inlen + len) <=
		    sizeof(curr->w_pwin->p_inbuf)) {
			bcopy(rbuf,
			      curr->w_pwin->p_inbuf +
...
struct mchar *mc;
{
	...
	if (mc->attr && ml->attr == null) {
		if ((ml->attr =
		     (unsigned char *) calloc(p->w_width + 1, 1)) == 0) {
			ml->attr = null;
...
static void MScrollH(p, n, y, xs, xe, bce)
struct win *p;
int n, y, xs, xe, bce;
{
	struct mline *ml;
...
static void MScrollV(p, n, ys, ye, bce)
struct win *p;
int n, ys, ye, bce;
{
	int i, cnt1, cnt2;
	...
	if (n > 0) {
		...
		cnt2 = (ye - ys + 1 - n) * sizeof(struct mline);
		if (cnt1 && cnt2)
			Scroll((char *) (p->w_mlines + ys), cnt2, cnt1,
			       (char *) tmp);
	}
}
...
static void MClearArea(p, xs, ys, xe, ye, bce)
struct win *p;
int xs, ys, xe, ye, bce;
{
	int n, y;
...
static void MWrapChar(p, c, y, top, bot, ins)
struct win *p;
struct mchar *c;
int y, top, bot;
int ins;
...
static void MPutStr(p, s, n, r, x, y)
struct win *p;
char *s;
int n;
struct mchar *r;
...
static void MBceLine(p, y, xs, xe, bce)
struct win *p;
int y, xs, xe, bce;
{
	struct mchar mc;
...
static void WAddLineToHist(wp, ml)
struct win *wp;
struct mline *ml;
{
...
int ys, ye;
{
	...
	for (y = ye; y >= ys; y--, ml--) {
		if (bcmp((char *) ml->image, blank, p->w_width))
			break;
		if (ml->attr != null
		    && bcmp((char *) ml->attr, null, p->w_width))
			break;
		if (ml->color != null
		    && bcmp((char *) ml->color, null, p->w_width))
			break;
#ifdef COLORS256
		if (ml->colorx != null
		    && bcmp((char *) ml->colorx, null, p->w_width))
			break;
#endif
	}
 ...
 */
static void WReverseVideo(p, on)
struct win *p;
int on;
{
	struct canvas *cv;
...
static int WindowChangedCheck(s, what, hp)
char *s;
int what;
int *hp;
{
...
int what;
{
	...
	if (p == 0) {
		for (display = displays; display;
			...
			for (cv = D_cvlist; cv; cv = cv->c_next) {
				if (inlstr
				    || (inlstrh && p && p->w_hstatus
					&& *p->w_hstatus
					&& WindowChangedCheck(p->w_hstatus,
							      what,
							      (int *) 0)))
					WListUpdatecv(cv,
						      (struct win *) 0);
				...
				if (inwstr
				    || (inwstrh && p && p->w_hstatus
					&& *p->w_hstatus
					&& WindowChangedCheck(p->w_hstatus,
							      what,
							      (int *) 0)))
					if (cv->c_ye + 1 < D_height)
						RefreshLine(cv->c_ye + 1,
							    0, D_width - 1,
							    0);
			}
			p = D_fore;
			if (inhstr
			    || (inhstrh && p && p->w_hstatus
				&& *p->w_hstatus
				&& WindowChangedCheck(p->w_hstatus, what,
						      (int *) 0)))
				RefreshHStatus();
			if (ox != -1 && ox != -1)
				GotoPos(ox, oy);
	...
	if (p->w_hstatus && *p->w_hstatus
	    && (inwstrh || inhstrh || inlstrh)
	...
	for (display = displays; display; display = display->d_next) {
		...
		for (cv = D_cvlist; cv; cv = cv->c_next) {
			...
			if (inwstr && cv->c_ye + 1 < D_height)
				RefreshLine(cv->c_ye + 1, 0, D_width - 1,
					    0);
		}
