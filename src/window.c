...
static int DoAutolf(buf, lenp, fr)
char *buf;
int *lenp;
int fr;
{
...
static void WinProcess(bufpp, lenp)
char **bufpp;
int *lenp;
{
	...
	if (display && ((fore->w_wlock == WLOCK_OFF) ?
		...
		debug2("window %d, user %s: ", fore->w_number,
		       D_user->u_name);
		debug2("writelock %d (wlockuser %s)\n", fore->w_wlock,
		       fore->w_wlockuser ? fore->w_wlockuser->
		       u_name : "NULL");
	...
	if (fore->w_type == W_TYPE_TELNET && TelIsline(fore)
	    && *bufpp != fore->w_telbuf) {
...
#endif
	{
		...
		if (fore->w_type == W_TYPE_TELNET
		    && (trunc = DoTelnet(ibuf + *ilen, &l2, f - l2))) {
			l -= trunc;
			if (fore->w_autolf)
...
static void ZombieProcess(bufpp, lenp)
char **bufpp;
int *lenp;
{
	int l = *lenp;
...
static void WinRedisplayLine(y, from, to, isblank)
int y, from, to, isblank;
{
	...
	fore = (struct win *) flayer->l_data;
	if (from == 0 && y > 0
	    && fore->w_mlines[y - 1].image[fore->w_width] == 0)
		LCDisplayLineWrap(&fore->w_layer, &fore->w_mlines[y], y,
				  from, to, isblank);
	else
		LCDisplayLine(&fore->w_layer, &fore->w_mlines[y], y, from,
			      to, isblank);
}

static int WinRewrite(y, x1, x2, rend, doit)
int y, x1, x2, doit;
struct mchar *rend;
{
	...
	if (is_dw_font(rend->font))
		return EXPENSIVE;
#ifdef UTF8
	if (fore->w_encoding && fore->w_encoding != UTF8
	    && D_encoding == UTF8
	    && ContainsSpecialDeffont(fore->w_mlines + y, x1, x2,
				      fore->w_encoding))
		return EXPENSIVE;
#endif
...
static void WinClearLine(y, xs, xe, bce)
int y, xs, xe, bce;
{
	fore = (struct win *) flayer->l_data;
...
int bufsize;
{
	...
	if ((w->w_log =
	     logfopen(buf,
		      islogfile(buf) ? NULL : secfopen(buf, "a"))) == NULL)
		return -2;
...
struct NewWindow *newwin;
{
...
#if defined(UTMP_USRLIMIT) && defined(UTMPOK)
	/*
	 * Count current number of users, if logging windows in.
	 */
	if (nwin.lflag && CountUsers() >= UTMP_USRLIMIT) {
		Msg(0,
		    "User limit reached.  Window will not be logged in.");
		nwin.lflag = 0;
	}
#endif
	n = pp - wtab;
	...
	p->w_flow =
	    nwin.flowflag | ((nwin.flowflag & FLOW_AUTOFLAG)
			     ? (FLOW_AUTO | FLOW_NOW) : FLOW_AUTO);
	if (!nwin.aka)
	...
	if (ChangeWindowSize(p, display ? D_defwidth : 80,
			     display ? D_defheight : 24, nwin.histheight))
	{
		FreeWindow(p);
		return -1;
	}
#else
	if (ChangeWindowSize
	    (p, display ? D_forecv->c_xe - D_forecv->c_xs + 1 : 80,
	...
	if (VerboseCreate && type != W_TYPE_GROUP) {
		...
		for (f = 0; p->w_cmdargs[f]; f++) {
			WriteString(p, " ", 1);
			WriteString(p, p->w_cmdargs[f],
				    strlen(p->w_cmdargs[f]));
		}
		WriteString(p, "\r\n", 2);
		display = d;
	}
...
#else				/* UTMP_LOGOUTOK */
	debug1
	    ("MakeWindow will log in, UTMP_LOGOUTOK undefined in config.h%s.\n",
...
struct NewWindow *nwin;
{
	...
	if (!nwin->aka) {
		Msg(0,
		    "Window lookup impossible without title, creating new.");
...
struct win *p;
{
	...
	if ((f =
	     OpenDevice(p->w_cmdargs, lflag, &p->w_type, &TtyName)) < 0)
		return -1;
...
struct win *wp;
{
	...
	if (wp->w_type == W_TYPE_PTY) {
		...
		if (chown(wp->w_tty, 0, 0))
			debug2("%s unable to chown errno %d\n", wp->w_tty, errno);
	}
	close(wp->w_ptyfd);
	wp->w_ptyfd = -1;
	wp->w_tty[0] = 0;
...
static int OpenDevice(args, lflag, typep, namep)
char **args;
int lflag;
int *typep;
char **namep;
 ...
 *
 */
static int ForkWindow(win, args, ttyn)
struct win *win;
char **args, *ttyn;
{
	int pid;
	...
	case 0:
		...
		strncpy(shellbuf + 6, ShellProg + (*ShellProg == '-'),
			sizeof(shellbuf) - 7);
		shellbuf[sizeof(shellbuf) - 1] = 0;
		NewEnv[4] = shellbuf;
		debug1("ForkWindow: NewEnv[4] = '%s'\n", shellbuf);
		if (win->w_term && *win->w_term
		    && strcmp(screenterm, win->w_term)
...
char *prog, **args, **env;
{
	...
	do {
		...
		case ENOEXEC:
			...
			shargs[1] = buf;
			for (i = 1; (shargs[i + 1] = args[i]) != NULL;
			     ++i);
...
char **av;
{
	...
	if ((w = display ? fore : windows) == NULL)
		return -1;
	if (!*av || w->w_pwin) {
		Msg(0, "Filter running: %s",
		    w->w_pwin ? w->w_pwin->p_cmd : "(none)");
		return -1;
	}
	if (w->w_ptyfd < 0) {
	...
	if (!
	    (pwin =
	     (struct pseudowin *) calloc(1, sizeof(struct pseudowin)))) {
		Msg(0, "%s", strnomem);
		return -1;
	}
	...
	*--t = '\0';
	...
	if (pwin->
	    p_fdpat & (F_PFRONT << F_PSHIFT * 2 | F_PFRONT << F_PSHIFT))
		evenq(&pwin->p_readev);
 ...
 */
int ReleaseAutoWritelock(dis, w)
struct display *dis;
struct win *w;
{
	...
	if (w->w_wlock == WLOCK_AUTO && w->w_wlockuser == dis->d_user) {
		...
		debug3("%s %s autolock on win %d\n",
		       dis->d_user->u_name, d ? "keeps" : "releases",
		       w->w_number);
		if (!d) {
			w->w_wlockuser = NULL;
			return 0;
 ...
 */
int ObtainAutoWritelock(d, w)
struct display *d;
struct win *w;
{
	if ((w->w_wlock == WLOCK_AUTO) &&
	    !AclCheckPermWin(d->d_user, ACL_WRITE, w) && !w->w_wlockuser) {
		debug2
...
static void paste_slowev_fn(ev, data)
struct event *ev;
char *data;
{
...
static void win_readev_fn(ev, data)
struct event *ev;
char *data;
{
...
static void win_writeev_fn(ev, data)
struct event *ev;
char *data;
{
	struct win *p = (struct win *) data;
	...
	if (p->w_paster.pa_pastelen && !p->w_slowpaste) {
		...
		flayer = pa->pa_pastelayer;
		if (flayer)
			DoProcess(p, &pa->pa_pasteptr, &pa->pa_pastelen,
				  pa);
	}
	return;
}
...
static void pseu_readev_fn(ev, data)
struct event *ev;
char *data;
{
	struct win *p = (struct win *) data;
...
static void pseu_writeev_fn(ev, data)
struct event *ev;
char *data;
{
	struct win *p = (struct win *) data;
...
static void win_silenceev_fn(ev, data)
struct event *ev;
char *data;
{
	struct win *p = (struct win *) data;
...
#ifdef MULTIUSER
		if (!
		    (ACLBYTE(p->w_lio_notify, D_user->u_id) &
		     ACLBIT(D_user->u_id)))
			continue;
#endif
		Msg(0, "Window %d: silence for %d seconds", p->w_number,
		    p->w_silencewait);
	}
}
